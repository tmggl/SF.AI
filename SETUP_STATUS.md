...
- **المرحلة الحالية:** **Phase 27.126 — Synonyms Reference Runtime Policy Design** (`PHASE27_126_SINALAB_SYNONYMS_REFERENCE_RUNTIME_POLICY_DESIGN_DECISION`; policy فقط؛ لا activation ولا تدريب)
...
- **نتيجة Phase 27.119:** dry-run counts لـ SinaLab Synonyms اكتملت: `3010` صفًا، `1093` مرشحًا مرجعيًا بعد الفلاتر، `685` مرشح eval؛ لا raw terms ولا corpus ولا tokenizer ولا training.
...
- **نتيجة Phase 27.126:** صُممت سياسة runtime مستقبلية بلا تفعيل: default `disabled`, aggregate-only output، منع raw terms/query rows/logs، ومنع template masking.
...
- **نتيجة Phase 27.93:** أضيف سطر renderer `الموضوع المطلوب: <topic_term>` لعائلة `topic` فقط، ومرّ dry-run للـ renderer/masking/canary.
...
- **نتيجة Phase 27.94:** أضيفت حزمة `10` سجلات سعودية `gold` لموضوع `الوفاء`، وأعيد بناء split؛ أصبح corpus `8453` (`msa=4199`, `saudi=4254`, `gold=3341`, `silver=5112`). بوابة 27.93 بعد الحزمة: `training_data_ready=true`, `shortfalls={}`, و`الوفاء=22` (`msa=12`, `saudi=10`).
...
- **نتيجة Phase 27.97:** صُمم objective `topic_copy_contrastive_binding_objective_v1`: ينسخ الموضوع المطلوب داخل أول 12 حرفًا عربيًا ظاهرًا من رد المساعد، ويمنع نجاح أي رد يذكر موضوعًا مجاورًا قبل المطلوب. القرار يسمح فقط ببوابة 27.98، لا تدريب.
- **تقرير Phase 27.97:** `docs/PHASE27_97_TOPIC_VARIABLE_BINDING_OBJECTIVE_DESIGN_REPORT.md`, `artifacts/reports/phase27_97_topic_variable_binding_objective_design_report.json`, `artifacts/reports/PHASE27_97_TOPIC_VARIABLE_BINDING_OBJECTIVE_DESIGN_DECISION.json`, `artifacts/reports/phase27_97_topic_variable_binding_objective_spec.json`.
- **نتيجة Phase 27.98:** رُمزت بوابة topic binding ونجحت آلية renderer/masking/canary، لكنها منعت التدريب لأن `500` سجل topic لا تحمل `topic_term` صريحًا. القرار: إصلاح metadata وcopy-anchor في Phase 27.99 بلا تدريب.
- **تقرير Phase 27.98:** `docs/PHASE27_98_TOPIC_BINDING_GATE_ENCODING_REPORT.md`, `artifacts/reports/phase27_98_topic_binding_gate_encoding_report.json`, `artifacts/reports/PHASE27_98_TOPIC_BINDING_GATE_ENCODING_DECISION.json`, `eval/prompts/phase27_98_topic_binding_contrastive_canary.json`.
- **نتيجة Phase 27.99:** أصلحنا `500` سجل topic من Phase 27.81 بإضافة `topic_term` صريح وتأكيد copy-anchor. إعادة بوابة 27.98 صارت `training_ready=true`, `metadata_ready=true`, `copy_anchor_ready=true`, و`missing_topic_term_records=0`. التالي تدريب 27.100 المقيّد فقط.
- **تقرير Phase 27.99:** `docs/PHASE27_99_TOPIC_METADATA_COPY_ANCHOR_REPAIR_REPORT.md`, `artifacts/reports/phase27_99_topic_metadata_copy_anchor_repair_report.json`, `artifacts/reports/PHASE27_99_TOPIC_METADATA_COPY_ANCHOR_REPAIR_DECISION.json`.
- **نتيجة Phase 27.100:** دُرّب إصلاح ربط الموضوع على `SF-10M` بشكل مقيّد. أفضل checkpoint `sf-10m-step1800` حقق known `13/16`, fresh `5/10`, copy-anchor `18/26`, reported wrong-topic `0`, topic-family `6/10`, all-family `37/50`. القرار: runtime محجوب والتالي تشخيص 27.101.
- **تقرير Phase 27.100:** `docs/PHASE27_100_BOUNDED_TOPIC_BINDING_REPAIR_REPORT.md`, `artifacts/reports/phase27_100_bounded_topic_binding_repair_report.json`, `artifacts/reports/PHASE27_100_BOUNDED_TOPIC_BINDING_REPAIR_DECISION.json`.
- **نتيجة Phase 27.101:** شُخّصت نتيجة 27.100 بلا تدريب. ظهر blind spot: reported wrong-topic `0` لكن observed wrong-topic `8` (`الصداقة=7`, `الامتنان=1`). القرار يسمح فقط ببوابة 27.102 لتصميم/ترميز topic prototype contrastive copy-anchor قبل أي تدريب.
- **تقرير Phase 27.101:** `docs/PHASE27_101_TOPIC_BINDING_RESULT_DIAGNOSIS_REPORT.md`, `artifacts/reports/phase27_101_topic_binding_result_diagnosis_report.json`, `artifacts/reports/PHASE27_101_TOPIC_BINDING_RESULT_DIAGNOSIS_DECISION.json`.
- **نتيجة Phase 27.102:** رُمزت بوابة observed wrong-topic/copy-anchor. البوابة التقطت فشل 27.100: reported wrong-topic `0` مقابل observed wrong-topic `8`, وcopy-anchor `18/26`. أُنشئ canary من `16` prompt. القرار يسمح فقط بحزمة curriculum في 27.103 بلا تدريب.
- **تقرير Phase 27.102:** `docs/PHASE27_102_TOPIC_PROTOTYPE_CONTRASTIVE_GATE_REPORT.md`, `artifacts/reports/phase27_102_topic_prototype_contrastive_gate_report.json`, `artifacts/reports/PHASE27_102_TOPIC_PROTOTYPE_CONTRASTIVE_GATE_DECISION.json`, `eval/prompts/phase27_102_topic_prototype_contrastive_canary.json`.
- **نتيجة Phase 27.103:** أضيفت حزمة `192` سجلًا `gold` لموضوعات prototype contrastive: `8` موضوعات × `2` لهجات × `12` سجلًا، مع copy-anchor أول الرد، وwrong-topic leak=`0`, وduplicate=`0`. corpus صار `8645` (`msa=4295`, `saudi=4350`, `gold=3533`, `silver=5112`) وsplit صار `train=7777`, `eval=868`. القرار يسمح فقط بتدريب 27.104 المحدود؛ runtime وSF-50M وtokenizer retrain محجوبة.
- **تقرير Phase 27.103:** `docs/PHASE27_103_TOPIC_PROTOTYPE_CONTRASTIVE_CURRICULUM_PACK_REPORT.md`, `artifacts/reports/phase27_103_topic_prototype_contrastive_curriculum_pack_report.json`, `artifacts/reports/PHASE27_103_TOPIC_PROTOTYPE_CONTRASTIVE_CURRICULUM_PACK_DECISION.json`, `data/corpus/chat/jsonl/dialogue_batch_v12_topic_prototype_contrastive_012.jsonl`.
- **نتيجة Phase 27.104:** دُرّب إصلاح topic prototype contrastive محدود على `SF-10M` من checkpoint 27.100 باستخدام view مرتب من حزمة 27.103. أفضل checkpoint `sf-10m-step1200`: prototype `16/16`, observed wrong-topic `0`, known `16/16`, fresh `9/10`, topic-family `9/10`, all-family `30/50`. القرار التاريخي: runtime محجوب. القرار الحالي الأعلى: re-anchor عند Phase 27.79 بخطة objective/curriculum/decoding قبل أي تدريب.
- **تقرير Phase 27.104:** `docs/PHASE27_104_BOUNDED_TOPIC_PROTOTYPE_CONTRASTIVE_REPAIR_REPORT.md`, `artifacts/reports/phase27_104_bounded_topic_prototype_contrastive_repair_report.json`, `artifacts/reports/PHASE27_104_BOUNDED_TOPIC_PROTOTYPE_CONTRASTIVE_REPAIR_DECISION.json`, `artifacts/samples/phase27_104_bounded_topic_prototype_contrastive_repair.md`.
...
- الكود الحالي يعرض `Phase 27.126` في `/system/status` و`/health`; runtime المولّد العام لا يزال محجوبًا حتى تمر بوابات objective/curriculum/decoding.
...
- `test_chat_native_generator.py` — 29 tests (Phase 15 + Phase 25 canary routing + Phase 27.37 semantic topic guard + Phase 27.95 topic prompt alignment)
...
- `test_phase27_97_topic_variable_binding_objective_design.py` — 3 tests (Phase 27.97 objective design)
- `test_phase27_98_topic_binding_gate_encoding.py` — 3 tests (Phase 27.98 gate/metadata audit)
- `test_phase27_99_topic_metadata_copy_anchor_repair.py` — 2 tests (Phase 27.99 metadata repair)
- `test_phase27_100_bounded_topic_binding_repair.py` — 3 tests (Phase 27.100 training result)
- `test_phase27_101_topic_binding_result_diagnosis.py` — 3 tests (Phase 27.101 diagnosis)
- `test_phase27_102_topic_prototype_contrastive_gate.py` — 3 tests (Phase 27.102 gate)
- `test_phase27_103_topic_prototype_contrastive_curriculum_pack.py` — 3 tests (Phase 27.103 curriculum pack)
...
- **Phase 27.97:** تصميم objective ربط الموضوع — مكتمل؛ `topic_copy_contrastive_binding_objective_v1` جاهز للترميز الجاف في Phase 27.98.
- **Phase 27.98:** ترميز بوابة ربط الموضوع وتدقيق metadata — مكتمل؛ الترميز نجح لكن `500` سجل topic تحتاج `topic_term` صريحًا، والتالي Phase 27.99 لإصلاح البيانات دون تدريب.
- **Phase 27.99:** إصلاح metadata وcopy-anchor — مكتمل؛ `missing_topic_term_records=0` وبوابة 27.98 صارت تسمح بـ Phase 27.100 كتدريب مقيّد.
- **Phase 27.100:** تدريب إصلاح ربط الموضوع المقيّد — مكتمل؛ لم يمر gates (`13/16`, `5/10`, `18/26`, `37/50`)؛ runtime محجوب والتالي diagnosis.
- **Phase 27.101:** تشخيص نتيجة ربط الموضوع — مكتمل؛ كشف blind spot في wrong-topic metric، والتالي 27.102 gate بلا تدريب.
- **Phase 27.102:** بوابة topic prototype contrastive copy-anchor — مكتملة؛ canary/spec جاهزة والتالي 27.103 حزمة curriculum بلا تدريب.
- **Phase 27.103:** حزمة topic prototype contrastive curriculum — مكتملة؛ `192` سجلًا gold جاهزة لمرحلة 27.104 كتدريب محدود.
- **Phase 27.104:** تدريب topic prototype contrastive محدود — مكتمل؛ topic gates نجحت لكن all-family تراجعت إلى `30/50`. هذا هو دليل re-anchor الحالي عند Phase 27.79.

أول توليد خام حدث في Phase 13. Phase 15 جهّز الباب داخل الشات، وPhase 16 أثبت أن التوليد مكرر. Phase 27.78 غيّرت المنهج: لا مزيد من التدريب المتكرر قبل تشخيص root-cause. Phase 27.79 صممت إصلاح objective/curriculum/decoding/family balance. Phase 27.80 شفّرت البوابات، Phase 27.81 عالجت توازن family ببيانات gold، Phase 27.82 سمحت بتدريب مقيّد، Phase 27.83 أثبتت أن الإصلاح الحالي لا يكفي، Phase 27.84 حددت السبب، Phase 27.85 صممت الإشارة الصريحة، Phase 27.86 أثبتت أن الإشارة تظهر فعليًا داخل نص التدريب ومخفية عن loss، Phase 27.87 أثبتت أن التدريب المقيّد ما زال غير كافٍ للحوار العام، Phase 27.88 حددت أن ترتيب stream هو الخلل الأكبر، Phase 27.89 أصلحت بوابة الترتيب قبل أي تدريب جديد، Phase 27.90 رفعت النتيجة إلى `35/50`، Phase 27.91 أثبتت أن الضعف المتبقي topic-specific لا capacity عام، Phase 27.92 صممت إصلاح topic-objective، Phase 27.93 أثبتت الترميز الجاف، Phase 27.94 سدّت فجوة `الوفاء` السعودية، Phase 27.95 أثبتت أن التدريب المحدود لم يكفِ، Phase 27.96 شخّصت السبب: خلل ربط متغير الموضوع لا نقص حجم، Phase 27.97 صممت objective copy/contrastive، Phase 27.98 منعت التدريب حتى إصلاح metadata، Phase 27.99 أصلحت metadata، Phase 27.100 درّبت إصلاح ربط الموضوع، Phase 27.101 كشف أن metric لم يلتقط prototype substitution، Phase 27.102 ثبّت بوابة observed wrong-topic، Phase 27.103 جهّزت حزمة curriculum مضادة للخلط، وPhase 27.104 درّبت إصلاحًا محدودًا نجح topic-wise لكنه فشل all-family.
