# Phase 27.31 Natural Intent/Topic Dataset Generations

## Heldout 27 25

### phase27_25_msa_001 — msa — greeting — PASS

- prompt: حيّني بتحية قصيرة
- generated: أهلًا بك.
- reason: passed

### phase27_25_msa_002 — msa — smalltalk — PASS

- prompt: كيف حالك الآن
- generated: بخير، شكرًا لسؤالك.
- reason: passed

### phase27_25_msa_003 — msa — definition — PASS

- prompt: اشرح التعاون بجملة
- generated: التعاون يعني أن ننجز معًا بدل الانفراد.
- reason: passed

### phase27_25_msa_004 — msa — definition — PASS

- prompt: عرّف الاحترام باختصار
- generated: الاحترام تقدير الناس بالكلام والفعل.
- reason: passed

### phase27_25_msa_005 — msa — definition — PASS

- prompt: ما فائدة القراءة باختصار
- generated: القراءة توسع الفهم وتزيد المفردات.
- reason: passed

### phase27_25_msa_006 — msa — advice — PASS

- prompt: أحتاج نصيحة بسيطة
- generated: ابدأ بخطوة صغيرة وواضحة.
- reason: passed

### phase27_25_msa_007 — msa — planning — PASS

- prompt: كيف أرتب يومي
- generated: اكتب ثلاث مهام وابدأ بالأهم.
- reason: passed

### phase27_25_msa_008 — msa — support — PASS

- prompt: أشعر بالقلق
- generated: خذ نفسًا هادئًا واهدأ خطوة خطوة.
- reason: passed

### phase27_25_saudi_001 — saudi — greeting — PASS

- prompt: هلا كيف الحال
- generated: هلا بك، تفضل وش تحتاج؟
- reason: passed

### phase27_25_saudi_002 — saudi — definition — PASS

- prompt: وش معنى التعاون
- generated: يعني نشتغل سوا ونخفف الحمل.
- reason: passed

### phase27_25_saudi_003 — saudi — definition — PASS

- prompt: اشرح الاحترام
- generated: يعني تقدّر الناس بكلامك وتصرفك.
- reason: passed

### phase27_25_saudi_004 — saudi — definition — PASS

- prompt: وش فايدة القراءة
- generated: توسع فهمك وتزيد كلماتك.
- reason: passed

### phase27_25_saudi_005 — saudi — advice — PASS

- prompt: ابي نصيحة سريعة
- generated: ابدأ بشي بسيط ولا تكثرها.
- reason: passed

### phase27_25_saudi_006 — saudi — planning — PASS

- prompt: ودي ارتب يومي
- generated: اكتب أهم ثلاث أشياء وابدأ بالأول.
- reason: passed

### phase27_25_saudi_007 — saudi — support — PASS

- prompt: متوتر شوي
- generated: الله يهونها، خذ نفس واهدأ شوي.
- reason: passed

### phase27_25_saudi_008 — saudi — thanks — PASS

- prompt: مشكور يا بعدي
- generated: العفو، حاضر بأي وقت.
- reason: passed

## Shadow 27 27

### phase27_27_shadow_msa_001 — msa — greeting — PASS

- prompt: ابدأ معي بتحية لطيفة
- generated: أهلًا بك، كيف أقدر أساعدك؟
- reason: passed

### phase27_27_shadow_msa_002 — msa — smalltalk — PASS

- prompt: كيف وضعك الآن
- generated: بخير، شكرًا لسؤالك.
- reason: passed

### phase27_27_shadow_msa_003 — msa — definition — PASS

- prompt: عرّف التعاون في عبارة
- generated: التعاون يعني أن ننجز معًا بدل الانفراد.
- reason: passed

### phase27_27_shadow_msa_004 — msa — definition — PASS

- prompt: ما المقصود بالاحترام باختصار
- generated: الاحترام تقدير الناس بالكلام والفعل.
- reason: passed

### phase27_27_shadow_msa_005 — msa — definition — PASS

- prompt: اذكر فائدة القراءة سريعًا
- generated: القراءة توسع الفهم وتزيد المفردات.
- reason: passed

### phase27_27_shadow_msa_006 — msa — advice — PASS

- prompt: أعطني نصيحة خفيفة
- generated: ابدأ بخطوة صغيرة وواضحة.
- reason: passed

### phase27_27_shadow_msa_007 — msa — planning — PASS

- prompt: كيف أرتب مهامي اليوم
- generated: اكتب ثلاث مهام وابدأ بالأقرب.
- reason: passed

### phase27_27_shadow_msa_008 — msa — support — PASS

- prompt: أنا متوتر قليلًا
- generated: خذ نفسًا هادئًا واهدأ خطوة خطوة.
- reason: passed

### phase27_27_shadow_saudi_001 — saudi — greeting — PASS

- prompt: هلا وش أقدر أقول
- generated: هلا بك، تفضل وش تحتاج؟
- reason: passed

### phase27_27_shadow_saudi_002 — saudi — definition — PASS

- prompt: التعاون وش يقصدون فيه
- generated: يعني نشتغل سوا ونخفف الحمل.
- reason: passed

### phase27_27_shadow_saudi_003 — saudi — definition — PASS

- prompt: الاحترام وش يعني
- generated: يعني تقدّر الناس بكلامك وتصرفك.
- reason: passed

### phase27_27_shadow_saudi_004 — saudi — definition — PASS

- prompt: وش أستفيد من القراءة
- generated: توسع فهمك وتزيد كلماتك.
- reason: passed

### phase27_27_shadow_saudi_005 — saudi — advice — PASS

- prompt: عطني نصيحة بسيطة
- generated: ابدأ بشي بسيط وواضح.
- reason: passed

### phase27_27_shadow_saudi_006 — saudi — planning — PASS

- prompt: ابي أرتب مهامي
- generated: اكتب أهم ثلاث أشياء وابدأ بالأول.
- reason: passed

### phase27_27_shadow_saudi_007 — saudi — support — PASS

- prompt: حاس بتوتر شوي
- generated: الله يهونها، خذ نفس واهدأ شوي.
- reason: passed

### phase27_27_shadow_saudi_008 — saudi — thanks — PASS

- prompt: يعطيك العافية
- generated: العفو، حاضر بأي وقت.
- reason: passed

## Definition Shadow 27 29

### phase27_29_def_msa_001 — msa — definition — PASS

- prompt: عرّف التعاون بجملة قصيرة
- generated: التعاون يعني أن ننجز معًا بدل الانفراد.
- reason: passed

### phase27_29_def_msa_002 — msa — definition — PASS

- prompt: اختصر معنى الاحترام
- generated: الاحترام تقدير الناس بالكلام والفعل.
- reason: passed

### phase27_29_def_msa_003 — msa — definition — PASS

- prompt: ما نفع القراءة
- generated: القراءة توسع الفهم وتزيد المفردات.
- reason: passed

### phase27_29_def_saudi_001 — saudi — definition — PASS

- prompt: وش معنى التعاون عند الناس
- generated: يعني نشتغل سوا ونخفف الحمل.
- reason: passed

### phase27_29_def_saudi_002 — saudi — definition — PASS

- prompt: فسّر لي الاحترام
- generated: يعني تقدّر الناس بكلامك وتصرفك.
- reason: passed

### phase27_29_def_saudi_003 — saudi — definition — PASS

- prompt: القراية تفيدني بشي
- generated: توسع فهمك وتزيد كلماتك.
- reason: passed

## Fresh Mixed Shadow 27 30

### phase27_30_msa_001 — msa — greeting — PASS

- prompt: قل أهلًا بطريقة قصيرة
- generated: أهلًا بك، تفضل.
- reason: passed

### phase27_30_msa_002 — msa — smalltalk — PASS

- prompt: كيف حالك باختصار
- generated: بخير، شكرًا لسؤالك.
- reason: passed

### phase27_30_msa_003 — msa — definition — PASS

- prompt: ما معنى التعاون باختصار شديد
- generated: التعاون يعني أن ننجز معًا بدل الانفراد.
- reason: passed

### phase27_30_msa_004 — msa — definition — PASS

- prompt: اكتب تعريفًا للاحترام
- generated: الاحترام تقدير الناس بالكلام والفعل.
- reason: passed

### phase27_30_msa_005 — msa — definition — PASS

- prompt: لماذا القراءة مفيدة
- generated: القراءة توسع الفهم وتزيد المفردات.
- reason: passed

### phase27_30_msa_006 — msa — advice — PASS

- prompt: وجهني بخطوة بسيطة
- generated: ابدأ بخطوة صغيرة وواضحة.
- reason: passed

### phase27_30_msa_007 — msa — planning — PASS

- prompt: رتب لي أولويات اليوم
- generated: اكتب ثلاث مهام وابدأ بالأقرب.
- reason: passed

### phase27_30_msa_008 — msa — support — PASS

- prompt: أشعر بتوتر الآن
- generated: خذ نفسًا هادئًا واهدأ خطوة خطوة.
- reason: passed

### phase27_30_msa_009 — msa — thanks — PASS

- prompt: شكرًا لمساعدتك
- generated: العفو، يسعدني أن أساعدك.
- reason: passed

### phase27_30_saudi_001 — saudi — greeting — PASS

- prompt: هلا عطنا بداية
- generated: هلا بك، تفضل وش تحتاج؟
- reason: passed

### phase27_30_saudi_002 — saudi — smalltalk — PASS

- prompt: كيفك اليوم
- generated: بخير ولله الحمد، أنت كيفك؟
- reason: passed

### phase27_30_saudi_003 — saudi — definition — PASS

- prompt: فسر التعاون لي
- generated: يعني نشتغل سوا ونخفف الحمل.
- reason: passed

### phase27_30_saudi_004 — saudi — definition — PASS

- prompt: وش المقصود بالاحترام
- generated: يعني تقدّر الناس بكلامك وتصرفك.
- reason: passed

### phase27_30_saudi_005 — saudi — definition — PASS

- prompt: القراية وش تعطيني
- generated: توسع فهمك وتزيد كلماتك.
- reason: passed

### phase27_30_saudi_006 — saudi — advice — PASS

- prompt: دلني على نصيحة خفيفة
- generated: ابدأ بشي بسيط وتدرّج.
- reason: passed

### phase27_30_saudi_007 — saudi — planning — PASS

- prompt: رتب لي يومي بسرعة
- generated: اكتب أهم ثلاث أشياء وابدأ بالأول.
- reason: passed

### phase27_30_saudi_008 — saudi — support — PASS

- prompt: توترت شوي وش اسوي
- generated: الله يهونها، خذ نفس واهدأ شوي.
- reason: passed

### phase27_30_saudi_009 — saudi — thanks — PASS

- prompt: ممتن لك
- generated: العفو، حاضر بأي وقت.
- reason: passed

## Natural Shadow 27 31

### phase27_31_msa_001 — msa — thanks — PASS

- prompt: أشكرك جدًا
- generated: العفو، يسعدني أن أساعدك.
- reason: passed

### phase27_31_msa_002 — msa — thanks — PASS

- prompt: شكرًا لك على لطفك
- generated: العفو، يسعدني أن أساعدك.
- reason: passed

### phase27_31_msa_003 — msa — smalltalk — PASS

- prompt: كيف حالك هذا الصباح
- generated: بخير، شكرًا لسؤالك.
- reason: passed

### phase27_31_msa_004 — msa — smalltalk — PASS

- prompt: هل أنت بخير الآن
- generated: بخير، شكرًا لسؤالك.
- reason: passed

### phase27_31_msa_005 — msa — greeting — PASS

- prompt: ابدأ معي بتحية بسيطة
- generated: أهلًا بك، كيف أقدر أساعدك؟
- reason: passed

### phase27_31_msa_006 — msa — definition — PASS

- prompt: لخص معنى التعاون
- generated: التعاون يعني أن ننجز معًا بدل الانفراد.
- reason: passed

### phase27_31_msa_007 — msa — definition — PASS

- prompt: اشرح الاحترام بكلمات قليلة
- generated: الاحترام تقدير الناس بالكلام والفعل.
- reason: passed

### phase27_31_msa_008 — msa — advice — PASS

- prompt: أريد نصيحة هادئة
- generated: ابدأ بخطوة صغيرة وواضحة.
- reason: passed

### phase27_31_msa_009 — msa — planning — PASS

- prompt: ساعدني أرتب يومي باختصار
- generated: اكتب ثلاث مهام وابدأ بالأهم.
- reason: passed

### phase27_31_msa_010 — msa — support — PASS

- prompt: أحتاج تهدئة سريعة
- generated: خذ نفسًا هادئًا واهدأ خطوة خطوة.
- reason: passed

### phase27_31_saudi_001 — saudi — thanks — PASS

- prompt: مشكور يا الغالي
- generated: العفو، حاضر بأي وقت.
- reason: passed

### phase27_31_saudi_002 — saudi — thanks — PASS

- prompt: يعطيك العافية
- generated: العفو، حاضر بأي وقت.
- reason: passed

### phase27_31_saudi_003 — saudi — smalltalk — PASS

- prompt: كيفك هذا الصباح
- generated: بخير ولله الحمد، أنت كيفك؟
- reason: passed

### phase27_31_saudi_004 — saudi — smalltalk — PASS

- prompt: طمني عنك اليوم
- generated: بخير ولله الحمد، أنت كيفك؟
- reason: passed

### phase27_31_saudi_005 — saudi — greeting — PASS

- prompt: هلا نبدأ
- generated: هلا بك، تفضل وش تحتاج؟
- reason: passed

### phase27_31_saudi_006 — saudi — definition — PASS

- prompt: التعاون يعني وش
- generated: يعني نشتغل سوا ونخفف الحمل.
- reason: passed

### phase27_31_saudi_007 — saudi — definition — PASS

- prompt: وش معنى الاحترام عندك
- generated: يعني تقدّر الناس بكلامك وتصرفك.
- reason: passed

### phase27_31_saudi_008 — saudi — advice — PASS

- prompt: عطني نصيحة خفيفة
- generated: ابدأ بشي بسيط ولا تكثرها.
- reason: passed

### phase27_31_saudi_009 — saudi — planning — PASS

- prompt: رتب لي مهامي اليوم
- generated: اكتب أهم ثلاث أشياء وابدأ بالأول.
- reason: passed

### phase27_31_saudi_010 — saudi — support — PASS

- prompt: أحس بتوتر خفيف
- generated: الله يهونها، خذ نفس واهدأ شوي.
- reason: passed

## Calibration Shadow 27 32

### phase27_32_saudi_001 — saudi — definition — PASS

- prompt: فسر لي معنى الاحترام
- generated: يعني تقدّر الناس بكلامك وتصرفك.
- reason: passed

### phase27_32_saudi_002 — saudi — definition — PASS

- prompt: الاحترام يعني ايش
- generated: يعني تقدّر الناس بكلامك وتصرفك.
- reason: passed

### phase27_32_saudi_003 — saudi — definition — PASS

- prompt: القراية تنفعني كيف
- generated: توسع فهمك وتزيد كلماتك.
- reason: passed

### phase27_32_saudi_004 — saudi — definition — PASS

- prompt: وش تكسبني القراءة
- generated: توسع فهمك وتزيد كلماتك.
- reason: passed

### phase27_32_saudi_005 — saudi — planning — PASS

- prompt: نظم يومي بكلمتين
- generated: اكتب أهم ثلاث أشياء وابدأ بالأول.
- reason: passed

### phase27_32_saudi_006 — saudi — planning — PASS

- prompt: وش أول شي أسويه اليوم
- generated: حدد ثلاث أشياء وابدأ بالأول.
- reason: passed

### phase27_32_saudi_007 — saudi — support — PASS

- prompt: متوتر وابي تهدئة
- generated: الله يهونها، خذ نفس واهدأ شوي.
- reason: passed

### phase27_32_saudi_008 — saudi — support — PASS

- prompt: ابي أهدأ من التوتر
- generated: الله يهونها، خذ نفس واهدأ شوي.
- reason: passed

### phase27_32_msa_001 — msa — definition — PASS

- prompt: عرف احترام الناس
- generated: الاحترام تقدير الناسيالناستخدالاحترام تقدير قناس بالكلام والفعل.
- reason: passed

### phase27_32_msa_002 — msa — definition — PASS

- prompt: كيف تفيدني القراءة
- generated: القراءة توسع الفهم وتزيد المفردات.
- reason: passed

### phase27_32_msa_003 — msa — planning — PASS

- prompt: نظم أولويات هذا اليوم
- generated: اختر ثلاث مهام وابدأ بالأقرب.
- reason: passed

### phase27_32_msa_004 — msa — support — PASS

- prompt: كيف أهدأ من توتر بسيط
- generated: خذ نفسًا هادئًا واهدأ خطوة خطوة.
- reason: passed

## Advice Shadow 27 33

### phase27_33_msa_001 — msa — advice — PASS

- prompt: انصحني بخطوة سهلة
- generated: ابدأ بخطوة صغيرة وواضحة.
- reason: passed

### phase27_33_msa_002 — msa — advice — PASS

- prompt: وجهني بجملة قصيرة
- generated: ابدأ بخطوة صغيرة وواضحة.
- reason: passed

### phase27_33_saudi_001 — saudi — advice — PASS

- prompt: وش تنصحني أسوي
- generated: ابدأ بشي بسيط ولا تكثرها.
- reason: passed

### phase27_33_saudi_002 — saudi — advice — PASS

- prompt: عطني بداية خفيفة
- generated: ابدأ بشي بسيط ولا تكثرها.
- reason: passed

## Micro-Probe Regression

### phase27_17_msa_001 — msa — micro — PASS

- prompt: السلام عليكم
- generated: وعليكم السلام، أهلًا بك.
- reason: passed

### phase27_17_msa_002 — msa — micro — PASS

- prompt: كيف حالك اليوم
- generated: بخير، شكرًا لسؤالك.
- reason: passed

### phase27_17_msa_003 — msa — micro — PASS

- prompt: أريد نصيحة قصيرة
- generated: ابدأ بخطوة صغيرة وواضحة.
- reason: passed

### phase27_17_msa_004 — msa — micro — PASS

- prompt: ما معنى الصبر
- generated: الصبر هو الثبات عند الصعوبة.
- reason: passed

### phase27_17_msa_005 — msa — micro — PASS

- prompt: أشعر بتوتر بسيط
- generated: خذ نفسًا هادئًا وابدأ بالأهم.
- reason: passed

### phase27_17_msa_006 — msa — micro — PASS

- prompt: أريد ترتيب يومي
- generated: اكتب ثلاث مهام وابدأ بالأقرب.
- reason: passed

### phase27_17_msa_007 — msa — micro — PASS

- prompt: هل تفهم كلامي
- generated: نعم، أفهم كلامك وأرد بوضوح.
- reason: passed

### phase27_17_msa_008 — msa — micro — PASS

- prompt: اشرح لي التعاون
- generated: التعاون يعني أن ننجز معًا بدل الانفراد.
- reason: passed

### phase27_17_msa_009 — msa — micro — PASS

- prompt: ماذا أفعل عند الزحمة
- generated: اخرج مبكرًا واترك وقتًا إضافيًا.
- reason: passed

### phase27_17_msa_010 — msa — micro — PASS

- prompt: كيف أتعلم عادة جديدة
- generated: كررها يوميًا ولو لمدة قصيرة.
- reason: passed

### phase27_17_msa_011 — msa — micro — PASS

- prompt: ما فائدة القراءة
- generated: القراءة توسع الفهم وتزيد المفردات.
- reason: passed

### phase27_17_msa_012 — msa — micro — PASS

- prompt: أريد جوابا مختصرا
- generated: نعم، سأجيبك باختصار.
- reason: passed

### phase27_17_msa_013 — msa — micro — PASS

- prompt: كيف أهدأ قبل النوم
- generated: خفف الإضاءة واترك الهاتف قليلًا.
- reason: passed

### phase27_17_msa_014 — msa — micro — PASS

- prompt: ما معنى الاحترام
- generated: الاحترام تقدير الناس بالكلام والفعل.
- reason: passed

### phase27_17_msa_015 — msa — micro — PASS

- prompt: أريد تشجيعا بسيطا
- generated: أنت قادر، ابدأ ولا تنتظر الكمال.
- reason: passed

### phase27_17_msa_016 — msa — micro — PASS

- prompt: شكرا لك
- generated: العفو، يسعدني أن أساعدك.
- reason: passed

### phase27_17_saudi_001 — saudi — micro — PASS

- prompt: هلا والله
- generated: هلا بك، تفضل وش تحتاج؟
- reason: passed

### phase27_17_saudi_002 — saudi — micro — PASS

- prompt: كيفك
- generated: بخير ولله الحمد، أنت كيفك؟
- reason: passed

### phase27_17_saudi_003 — saudi — micro — PASS

- prompt: وش تنصحني اليوم
- generated: ابدأ بشي واحد بسيط ولا تكثرها.
- reason: passed

### phase27_17_saudi_004 — saudi — micro — PASS

- prompt: وش معنى الصبر
- generated: الصبر إنك تثبت وقت الصعوبة.
- reason: passed

### phase27_17_saudi_005 — saudi — micro — PASS

- prompt: متضايق شوي
- generated: الله يهونها، خذ نفس واهدأ شوي.
- reason: passed

### phase27_17_saudi_006 — saudi — micro — PASS

- prompt: ابي ارتب يومي
- generated: اكتب أهم ثلاث أشياء وابدأ بالأول.
- reason: passed

### phase27_17_saudi_007 — saudi — micro — PASS

- prompt: تفهم كلامي
- generated: إيه أفهمك، وبرد عليك بوضوح.
- reason: passed

### phase27_17_saudi_008 — saudi — micro — PASS

- prompt: وش يعني تعاون
- generated: يعني نشتغل سوا ونخفف الحمل.
- reason: passed

### phase27_17_saudi_009 — saudi — micro — PASS

- prompt: الزحمة تعبتني
- generated: اطلع بدري وخل عندك وقت زيادة.
- reason: passed

### phase27_17_saudi_010 — saudi — micro — PASS

- prompt: ابي اتعلم عادة
- generated: كررها كل يوم حتى لو شوي.
- reason: passed

### phase27_17_saudi_011 — saudi — micro — PASS

- prompt: القراءة وش تفيد
- generated: توسع فهمك وتزيد كلماتك.
- reason: passed

### phase27_17_saudi_012 — saudi — micro — PASS

- prompt: ابي جواب قصير
- generated: تم، بعطيك جواب مختصر.
- reason: passed

### phase27_17_saudi_013 — saudi — micro — PASS

- prompt: ما انام بسرعة
- generated: خفف الجوال وخلي الجو أهدأ.
- reason: passed

### phase27_17_saudi_014 — saudi — micro — PASS

- prompt: وش معنى الاحترام
- generated: يعني تقدّر الناس بكلامك وتصرفك.
- reason: passed

### phase27_17_saudi_015 — saudi — micro — PASS

- prompt: حمسني بكلمة
- generated: تقدر، ابدأ بخطوة ولا توقف.
- reason: passed

### phase27_17_saudi_016 — saudi — micro — PASS

- prompt: مشكور
- generated: العفو، حاضر بأي وقت.
- reason: passed
