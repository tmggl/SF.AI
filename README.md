...
- **الأولوية الحالية:** تنفيذ توسعة corpus من `1050` إلى `5000` داخل `msa + saudi`; لا تدريب `SF-50M` ولا Phase 28 الآن.
- **الشات الحالي:** runtime rule-based + routing، وليس LLM مولّدًا بعد.
- **البيانات الحالية:** corpus موثق `1050` سجلًا يمر `corpus-audit`: `525` سعودي + `525` فصحى. آخر tokenizer v2 وSF-10M v0.2 استُخدما corpus Phase 22 عند `500` سجل، والتوسعة الجديدة بدأت بعد Phase 27.
...
- **قرار Phase 26:** `SF-50M` غير جاهز: corpus الحالي `1050` فقط والحد العملي `5000`، وruntime quality/hallucination/repetition gates لم تنجح بعد. التقرير: [docs/PHASE26_SF50M_READINESS_REPORT.md](./docs/PHASE26_SF50M_READINESS_REPORT.md).
- **نتيجة Phase 27:** `make phase27-dialogue-eval` مرّر `19/19` turn في suite متعدد الأدوار، لكن كل الردود `template`، و`open_generator_ready=false`. بعد Batch 002 صارت خطة التوسعة: `3950` سجلًا إضافيًا، `8` دفعات كبيرة تقريبًا، بالتساوي `msa=1975` و`saudi=1975`. التقرير: [docs/PHASE27_DIALOGUE_EVAL_V2_REPORT.md](./docs/PHASE27_DIALOGUE_EVAL_V2_REPORT.md).
- **دفعات توسعة:** أضيفت Batch 001 بإجمالي 50 سجلًا، ثم Batch 002 الكبيرة بإجمالي 500 سجل (`250` فصيح + `250` سعودي). التقارير: [docs/PHASE27_CORPUS_EXPANSION_BATCH_001_REPORT.md](./docs/PHASE27_CORPUS_EXPANSION_BATCH_001_REPORT.md), [docs/PHASE27_CORPUS_EXPANSION_BATCH_002_REPORT.md](./docs/PHASE27_CORPUS_EXPANSION_BATCH_002_REPORT.md).
