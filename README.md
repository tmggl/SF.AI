...
- **المولّد:** Phase 15 أضاف NativeGenerator adapter، لكن الواجهة المستقرة تبقى على `generator=template`; مختبر سامي المحلي يفعّله للتجربة عبر flags التشغيل فقط.
...
- **Corpus v2:** Phase 22 أضاف `make phase22-readiness` و`make phase22-plan` و`make phase22-review-intake`; الوضع الحالي 30/500 و`msa` ناقصة، وملفات review لا تدخل التدريب تلقائيًا. `phase22-review-intake` يعطي الآن score جودة للحوار قبل أي تحويل.
