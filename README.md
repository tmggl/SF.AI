...
- **البيانات الحالية:** corpus صغير `80/500` يمر `corpus-audit`: `30` سعودي + `50` فصحى؛ ما زال التوازن والعدد مطلوبين قبل تدريب جودة مفيد.
...
- **Corpus v2:** Phase 22 أضاف `make phase22-readiness` و`make phase22-plan` و`make phase22-next-batch` و`make phase22-completion-gate` و`make phase22-review-intake`; الوضع الحالي 80/500، وفيه دفعتان فصيحتان `dialogue_batch_v2_msa_001.jsonl` و`dialogue_batch_v2_msa_002.jsonl`. الواجهة تعرض بوابة Phase 22 الحية وجودة التصدير كمختبر اختياري فقط، أما بناء corpus الحالي فيتم مباشرة عبر الوكيل. `phase22-plan` يعطي batches مفصلة تبدأ الآن بـ `msa_003`، و`phase22-completion-gate` يمنع Phase 23 حتى اكتمال الشروط.
