...
- **البيانات الحالية:** corpus صغير `177/500` يمر `corpus-audit`: `30` سعودي + `147` فصحى؛ ما زال التوازن والعدد مطلوبين قبل تدريب جودة مفيد.
...
- **Corpus v2:** Phase 22 أضاف `make phase22-readiness` و`make phase22-plan` و`make phase22-next-batch` و`make phase22-completion-gate` و`make phase22-review-intake`; الوضع الحالي 177/500، وفيه خمس دفعات فصيحة `dialogue_batch_v2_msa_001.jsonl` إلى `dialogue_batch_v2_msa_005.jsonl` إضافة إلى seed فصيح للمصطلحات `protected_terms_msa_seed_v1.jsonl`. الواجهة تعرض بوابة Phase 22 الحية وجودة التصدير كمختبر اختياري فقط، أما بناء corpus الحالي فيتم مباشرة عبر الوكيل. `phase22-plan` يعطي batches مفصلة تبدأ الآن بـ `msa_006`، و`phase22-completion-gate` يمنع Phase 23 حتى اكتمال الشروط.
