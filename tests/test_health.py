...
def test_system_status_sovereign_flags() -> None:
    ...
    body = r.json()
    assert body["project"] == "SF.AI"
    assert body["current_phase"].startswith("Phase 27.52")
    assert body["current_phase_status"] == "completed_partial_keep_phase27_47_runtime"
    assert "Phase 27.53" in body["next_phase"]
