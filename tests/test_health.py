...
def test_system_status_sovereign_flags() -> None:
    ...
    body = r.json()
    assert body["project"] == "SF.AI"
    assert body["current_phase"].startswith("Phase 27.54")
    assert body["current_phase_status"] == "completed_full_scaling_blocked_diagnostic_micro_probe_allowed"
    assert "Phase 27.55" in body["next_phase"]
