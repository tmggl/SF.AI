...
def test_system_status_sovereign_flags() -> None:
    ...
    body = r.json()
    assert body["project"] == "SF.AI"
    assert body["current_phase"].startswith("Phase 27.91")
    assert body["current_phase_status"] == "phase27_91_diagnosed_topic_collapse_no_training"
    assert "Phase 27.92" in body["next_phase"]
