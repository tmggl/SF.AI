...
def test_system_status_sovereign_flags() -> None:
    ...
    body = r.json()
    assert body["project"] == "SF.AI"
    assert body["current_phase"].startswith("Phase 27.57")
    assert body["current_phase_status"] == "completed_repair_pack_ready_for_bounded_retraining_gate"
    assert "Phase 27.58" in body["next_phase"]
