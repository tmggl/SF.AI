...
def test_system_corpus_audit_reports_reviewed_seeds_ready() -> None:
    ...
    assert body["total_records"] == 550
    assert body["training_ready"] == 550
    assert body["issue_count"] == 0
    assert body["dialect_counts"] == {"msa": 275, "saudi": 275}
    assert body["quality_counts"] == {"gold": 52, "silver": 498}
...
def test_system_phase12_readiness_is_read_only_and_permission_gated() -> None:
    ...
    assert body["corpus_training_ready"] == 550
    assert body["corpus_issue_count"] == 0
    assert body["corpus_dialect_counts"] == {"msa": 275, "saudi": 275}
...
def test_system_phase23_tokenizer_audit_reports_v2_ready() -> None:
    ...
    assert body["corpus"]["training_ready"] == 550
    assert body["corpus"]["dialects"] == {"msa": 275, "saudi": 275}
