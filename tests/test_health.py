...
def test_system_status_sovereign_flags() -> None:
    ...
    body = r.json()
    assert body["project"] == "SF.AI"
    assert body["current_phase"].startswith("Phase 27.64")
    assert body["current_phase_status"] == "completed_topic_lexical_inspection_tokenizer_v8_required_runtime_blocked"
    assert "Phase 27.65" in body["next_phase"]
