...
def test_system_status_sovereign_flags() -> None:
    ...
    body = r.json()
    assert body["project"] == "SF.AI"
    assert body["current_phase"].startswith("Phase 27.15")
    assert body["current_phase_status"] == "completed_eval_improved_strict_generation_blocked"
