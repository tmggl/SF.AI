...
def test_system_corpus_audit_reports_reviewed_seeds_ready() -> None:
    ...
    assert body["total_records"] == 55
    assert body["training_ready"] == 55
    assert body["issue_count"] == 0
    assert body["dialect_counts"] == {"msa": 25, "saudi": 30}
    assert body["quality_counts"] == {"gold": 30, "silver": 25}
...
def test_system_phase12_readiness_is_read_only_and_permission_gated() -> None:
    ...
    assert body["corpus_training_ready"] == 55
    assert body["corpus_issue_count"] == 0
    assert body["corpus_dialect_counts"] == {"msa": 25, "saudi": 30}
