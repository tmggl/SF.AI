...
def test_system_status_sovereign_flags() -> None:
    ...
    body = r.json()
    assert body["project"] == "SF.AI"
    assert body["current_phase"].startswith("Phase 27.55")
    assert body["current_phase_status"] == "completed_diagnostic_capacity_signal_failed_full_sf50m_blocked"
    assert "Phase 27.56" in body["next_phase"]
