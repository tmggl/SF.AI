...
def test_system_status_sovereign_flags() -> None:
    ...
    body = r.json()
    assert body["project"] == "SF.AI"
    assert body["current_phase"].startswith("Phase 27.65")
    assert body["current_phase_status"] == "passed_tokenizer_v8_topic_probe_runtime_blocked"
    assert "Phase 27.66" in body["next_phase"]
