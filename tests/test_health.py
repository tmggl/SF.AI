...
def test_system_status_sovereign_flags() -> None:
    ...
    body = r.json()
    assert body["project"] == "SF.AI"
    assert body["current_phase"].startswith("Phase 27.56")
    assert body["current_phase_status"] == "completed_objective_format_tokenizer_diagnosis_runtime_blocked"
    assert "Phase 27.57" in body["next_phase"]
