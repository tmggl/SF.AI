...
def test_health_ok() -> None:
    ...
    assert body["project"] == "SF.AI"
    assert body["phase"] == "Phase 27.118"
...
def test_system_status_sovereign_flags() -> None:
    ...
    body = r.json()
    assert body["project"] == "SF.AI"
    assert body["current_phase"].startswith("Phase 27.118")
    assert body["current_phase_status"] == (
        "phase27_118_synonyms_reference_extraction_design_ready_no_import"
    )
    assert "Phase 27.119" in body["next_phase"]
