...
def test_system_status_sovereign_flags() -> None:
    ...
    body = r.json()
    assert body["project"] == "SF.AI"
    assert body["current_phase"].startswith("Phase 26")
    assert body["current_phase_status"] == "completed_not_ready_expand_corpus_and_improve_sf10m"
    assert body["next_phase"].startswith("Phase 27")
...
def test_system_corpus_audit_reports_reviewed_seeds_ready() -> None:
    ...
    assert body["training_ready"] == 500
    assert body["issue_count"] == 0
    assert body["dialect_counts"] == {"msa": 250, "saudi": 250}
    assert body["quality_counts"] == {"gold": 52, "silver": 448}
...
def test_system_phase12_readiness_is_read_only_and_permission_gated() -> None:
    ...
    assert body["corpus_training_ready"] == 500
    assert body["corpus_issue_count"] == 0
    assert body["corpus_dialect_counts"] == {"msa": 250, "saudi": 250}
