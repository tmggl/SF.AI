...
def test_system_status_sovereign_flags() -> None:
    ...
    body = r.json()
    assert body["project"] == "SF.AI"
    assert body["current_phase"].startswith("Phase 27.67")
    assert body["current_phase_status"] == "failed_fresh_shadow_canary_runtime_blocked"
    assert "Phase 27.68" in body["next_phase"]
