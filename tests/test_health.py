...
def test_system_status_sovereign_flags() -> None:
    ...
    body = r.json()
    assert body["project"] == "SF.AI"
    assert body["current_phase"].startswith("Phase 27.66")
    assert body["current_phase_status"] == "passed_v8_bounded_topic_repair_runtime_blocked"
    assert "Phase 27.67" in body["next_phase"]
