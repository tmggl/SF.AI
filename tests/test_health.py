...
def test_system_status_sovereign_flags() -> None:
    ...
    body = r.json()
    assert body["project"] == "SF.AI"
    assert body["current_phase"].startswith("Phase 27.22")
    assert body["current_phase_status"] == "completed_partial_spacing_boundary_repair_runtime_blocked"
    assert "semantic" in body["next_phase"]
