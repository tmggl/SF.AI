...
def test_health_ok() -> None:
    ...
    assert body["project"] == "SF.AI"
    assert body["phase"] == "Phase 27.124"
...
def test_system_status_sovereign_flags() -> None:
    ...
    assert any(
        c["name"] == "phase27_70_open_social_repair"
        and c["status"] == "failed_open_social_repair_runtime_blocked"
        for c in body["components"]
    )
    assert any(
        c["name"] == "phase27_71_candidate_selection"
        and c["status"] == "no_stable_candidate_runtime_blocked"
        for c in body["components"]
    )
    assert any(
        c["name"] == "phase27_72_stability_first_repair"
        and c["status"] == "improved_stability_first_repair_runtime_blocked"
        for c in body["components"]
    )
    assert any(
        c["name"] == "phase27_73_open_social_failure_inspection"
        and c["status"] == "completed_open_social_failure_inspection_runtime_blocked"
        for c in body["components"]
    )
    assert any(
        c["name"] == "phase27_74_open_social_semantic_collapse_repair"
        and c["status"] == "failed_open_social_semantic_collapse_repair_runtime_blocked"
        for c in body["components"]
    )
    assert any(
        c["name"] == "phase27_75_open_social_strategy_inspection"
        and c["status"] == "completed_open_social_strategy_inspection_runtime_blocked"
        for c in body["components"]
    )
    assert any(
        c["name"] == "phase27_76_tokenizer_v9_open_social_boundary_probe"
        and c["status"] == "passed_tokenizer_v9_open_social_boundary_probe_runtime_blocked"
        for c in body["components"]
    )
    assert any(
        c["name"] == "phase27_77_v9_bounded_open_social_lm_repair"
        and c["status"] == "failed_v9_bounded_open_social_lm_repair_runtime_blocked"
        for c in body["components"]
    )
    assert any(
        c["name"] == "phase27_78_engineering_root_cause_gate"
        and c["status"] == "phase27_78_engineering_decision_training_blocked"
        for c in body["components"]
    )
    assert any(
    ...
    assert any(
        c["name"] == "phase27_81_bounded_family_conditioned_repair_training"
        and c["status"] == "trained_runtime_blocked_diagnosis_required"
        for c in body["components"]
    )
    assert any(
    ...
    assert any(
        c["name"] == "phase27_105_raw_ui_lab_result_diagnosis"
        and c["status"] == "diagnosed_raw_ui_lab_failures_no_training"
        for c in body["components"]
    )
    assert any(
        c["name"] == "phase27_106_social_subfamily_topic_variant_design"
        and c["status"] == "design_ready_gate_encoding_no_training"
        for c in body["components"]
    )
    assert any(
        c["name"] == "phase27_107_social_subfamily_topic_variant_gate"
        and c["status"] == "gate_passed_data_pack_allowed_no_training"
        for c in body["components"]
    )
    assert any(
        c["name"] == "phase27_123_sinalab_synonyms_reference_adapter_design"
        and c["status"] == "synonyms_reference_adapter_design_ready_no_runtime"
        for c in body["components"]
    )
    assert any(
        c["name"] == "phase27_124_sinalab_synonyms_reference_adapter_skeleton"
        and c["status"] == "synonyms_reference_adapter_skeleton_ready_no_runtime"
        for c in body["components"]
    )
    assert any(
...
def test_system_corpus_audit_reports_reviewed_seeds_ready() -> None:
    ...
    assert body["total_records"] == 9125
    assert body["training_ready"] == 9125
    assert body["issue_count"] == 0
    assert body["dialect_counts"] == {"msa": 4535, "saudi": 4590}
    assert body["quality_counts"] == {"gold": 4013, "silver": 5112}
...
def test_system_phase12_readiness_is_read_only_and_permission_gated() -> None:
    ...
    assert body["corpus_training_ready"] == 9125
    assert body["corpus_issue_count"] == 0
    assert body["corpus_dialect_counts"] == {"msa": 4535, "saudi": 4590}
...
def test_system_phase23_tokenizer_audit_reports_v2_ready() -> None:
    ...
    assert body["corpus"]["training_ready"] == 9125
    assert body["corpus"]["dialects"] == {"msa": 4535, "saudi": 4590}
