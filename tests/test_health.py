...
def test_health_ok() -> None:
    ...
    assert body["project"] == "SF.AI"
    assert body["phase"] == "Phase 27.111"
...
def test_system_status_sovereign_flags() -> None:
    ...
    body = r.json()
    assert body["project"] == "SF.AI"
    assert body["current_phase"].startswith("Phase 27.111")
    assert body["current_phase_status"] == (
        "phase27_111_qabas_bootstrap_design_ready_import_blocked"
    )
    assert "Phase 27.112" in body["next_phase"]
