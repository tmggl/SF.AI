...
def test_system_status_sovereign_flags() -> None:
    ...
    body = r.json()
    assert body["project"] == "SF.AI"
    assert body["current_phase"].startswith("Phase 27.84")
    assert body["current_phase_status"] == "phase27_84_diagnosed_objective_curriculum_failure_no_training"
    assert "Phase 27.85" in body["next_phase"]
