...
def test_system_status_sovereign_flags() -> None:
    ...
    body = r.json()
    assert body["project"] == "SF.AI"
    assert body["current_phase"].startswith("Phase 27.10")
    assert body["current_phase_status"] == "completed_numeric_improvement_generation_still_blocked"
...
def test_system_corpus_audit_reports_reviewed_seeds_ready() -> None:
    ...
    assert body["total_records"] == 5543
    assert body["training_ready"] == 5543
    assert body["issue_count"] == 0
    assert body["dialect_counts"] == {"msa": 2749, "saudi": 2794}
    assert body["quality_counts"] == {"gold": 431, "silver": 5112}
...
def test_system_phase12_readiness_is_read_only_and_permission_gated() -> None:
    ...
    assert body["corpus_training_ready"] == 5543
    assert body["corpus_issue_count"] == 0
    assert body["corpus_dialect_counts"] == {"msa": 2749, "saudi": 2794}
...
def test_system_phase19_readiness_reports_training_gate() -> None:
    ...
    assert body["training_records"] == 5543
...
def test_system_phase23_tokenizer_audit_reports_v2_ready() -> None:
    ...
    assert body["corpus"]["training_ready"] == 5543
    assert body["corpus"]["dialects"] == {"msa": 2749, "saudi": 2794}
