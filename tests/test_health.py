...
def test_system_status_sovereign_flags() -> None:
    ...
    body = r.json()
    assert body["project"] == "SF.AI"
    assert body["current_phase"].startswith("Phase 27.58")
    assert body["current_phase_status"] == "failed_bounded_alignment_probe_runtime_blocked"
    assert "Phase 27.59" in body["next_phase"]
