...
def test_system_status_sovereign_flags() -> None:
    ...
    body = r.json()
    assert body["project"] == "SF.AI"
    assert body["current_phase"].startswith("Phase 27.76")
    assert body["current_phase_status"] == "passed_tokenizer_v9_open_social_boundary_probe_runtime_blocked"
    assert "Phase 27.77" in body["next_phase"]
