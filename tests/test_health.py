...
def test_system_status_sovereign_flags() -> None:
    ...
    body = r.json()
    assert body["project"] == "SF.AI"
    assert body["current_phase"].startswith("Phase 27.89")
    assert body["current_phase_status"] == "phase27_89_stratified_round_robin_sampler_gate_passed_training_allowed_next"
    assert "Phase 27.90" in body["next_phase"]
    ...
    assert any(
        ...
        and c["status"] == "phase27_79_repair_design_ready_next_gate_encoding_no_training"
        for c in body["components"]
    )
    assert any(
...
def test_system_corpus_audit_reports_reviewed_seeds_ready() -> None:
    ...
    assert body["total_records"] == 8443
    assert body["training_ready"] == 8443
    assert body["issue_count"] == 0
    assert body["dialect_counts"] == {"msa": 4199, "saudi": 4244}
    assert body["quality_counts"] == {"gold": 3331, "silver": 5112}
...
def test_system_phase12_readiness_is_read_only_and_permission_gated() -> None:
    ...
    assert body["corpus_training_ready"] == 8443
    assert body["corpus_issue_count"] == 0
    assert body["corpus_dialect_counts"] == {"msa": 4199, "saudi": 4244}
...
def test_system_phase19_readiness_reports_training_gate() -> None:
    ...
    assert body["training_records"] == 8443
...
def test_system_phase23_tokenizer_audit_reports_v2_ready() -> None:
    ...
    assert body["corpus"]["training_ready"] == 8443
    assert body["corpus"]["dialects"] == {"msa": 4199, "saudi": 4244}
