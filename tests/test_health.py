...
def test_system_status_sovereign_flags() -> None:
    ...
    body = r.json()
    assert body["project"] == "SF.AI"
    assert body["current_phase"].startswith("Phase 27.42")
    assert body["current_phase_status"] == "completed_live_ui_broader_probes_guarded"
    assert "Phase 27.43" in body["next_phase"]
