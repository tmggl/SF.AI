...
def test_system_corpus_audit_reports_reviewed_seeds_ready() -> None:
    ...
    assert body["total_records"] == 355
    assert body["training_ready"] == 355
    assert body["issue_count"] == 0
    assert body["dialect_counts"] == {"msa": 200, "saudi": 155}
    assert body["quality_counts"] == {"gold": 52, "silver": 303}
...
def test_system_phase12_readiness_is_read_only_and_permission_gated() -> None:
    ...
    assert body["corpus_training_ready"] == 355
    assert body["corpus_issue_count"] == 0
    assert body["corpus_dialect_counts"] == {"msa": 200, "saudi": 155}
