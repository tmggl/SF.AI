...
def test_system_status_sovereign_flags() -> None:
    ...
    body = r.json()
    assert body["project"] == "SF.AI"
    assert body["current_phase"].startswith("Phase 27.21")
    assert body["current_phase_status"] == "completed_tokenizer_v3_micro_probe_runtime_blocked"
    assert "spacing" in body["next_phase"]
