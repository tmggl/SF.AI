...
def test_phase22_collection_plan_calculates_real_quotas() -> None:
    ...
    assert len(plan.planned_batches) == 16
    assert plan.planned_batches[0].batch_id == "msa_004"
...
def test_phase22_collection_plan_endpoint() -> None:
    ...
    assert body["planned_batches"][0]["suggested_output_path"].endswith("dialogue_batch_v2_msa_004.jsonl")
...
def test_phase22_next_batch_brief_points_to_next_msa_batch() -> None:
    ...
    assert brief.next_batch.batch_id == "msa_004"
    ...
    assert "validate_dataset.py data/corpus/chat/jsonl/dialogue_batch_v2_msa_004.jsonl" in brief.after_export_commands[0]
...
def test_phase22_next_batch_endpoint() -> None:
    ...
    assert body["next_batch"]["batch_id"] == "msa_004"
    assert body["next_batch"]["dialect"] == "msa"
    assert body["next_batch"]["target_records"] == 25
    assert len(body["suggested_topics"]) >= 80
    assert body["warnings"]
    assert "validate_dataset.py data/corpus/chat/jsonl/dialogue_batch_v2_msa_004.jsonl" in body["after_export_commands"][0]
...
def test_phase22_completion_gate_blocks_advancement_until_complete() -> None:
    ...
    assert "complete_next_batch:msa_004" in gate.missing_requirements
...
def test_phase22_completion_gate_endpoint() -> None:
    ...
    assert body["current_next_batch"] == "msa_004"
    assert body["completion_checks"]["corpus_target_met"] is False
    assert "complete_next_batch:msa_004" in body["missing_requirements"]
...
def test_phase22_collection_plan_cli_is_read_only() -> None:
    ...
    assert "estimated_batches             : 8" in proc.stdout
    assert "planned batches:" in proc.stdout
    assert "#01 msa_004 (msa, 50 records, minimum_msa)" in proc.stdout
    assert "synthetic_llm_data_allowed    : false" in proc.stdout
...
def test_phase22_next_batch_cli_is_read_only() -> None:
    ...
    assert "next_batch                    : msa_004" in proc.stdout
...
def test_phase22_completion_gate_cli_is_read_only() -> None:
    ...
    assert "current_next_batch            : msa_004" in proc.stdout
