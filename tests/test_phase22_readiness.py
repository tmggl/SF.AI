...
def test_phase22_readiness_reports_current_gap() -> None:
    ...
    assert decision.training_records == 330
    assert decision.target_records == 500
    assert decision.remaining_records == 170
    assert decision.dialect_counts == {"msa": 200, "saudi": 130}
...
def test_phase22_readiness_requires_balance_before_phase23() -> None:
    ...
    assert decision.dialect_shortfalls["saudi"] == 70
...
def test_phase22_endpoint() -> None:
    ...
    assert body["training_records"] == 330
...
def test_phase22_collection_plan_calculates_real_quotas() -> None:
    ...
    assert plan.current_records == 330
    assert plan.remaining_records == 170
    assert plan.batch_size == 25
    assert plan.estimated_batches == 7
    assert plan.quota_by_dialect == {"saudi": 70}
    ...
    assert len(plan.planned_batches) == 7
    assert plan.planned_batches[0].batch_id == "saudi_005"
    assert plan.planned_batches[0].dialect == "saudi"
    assert plan.planned_batches[0].target_records == 25
    assert plan.planned_batches[2].batch_id == "saudi_007"
    assert plan.planned_batches[2].target_records == 20
...
def test_phase22_collection_plan_endpoint() -> None:
    ...
    assert body["estimated_batches"] == 4
    assert body["quota_by_dialect"] == {"saudi": 70}
    assert body["synthetic_llm_data_allowed"] is False
    assert len(body["planned_batches"]) == 4
    assert body["planned_batches"][0]["batch_id"] == "saudi_005"
    assert body["planned_batches"][0]["suggested_output_path"].endswith("dialogue_batch_v2_saudi_005.jsonl")
...
def test_phase22_next_batch_brief_points_to_next_saudi_batch() -> None:
    ...
    assert brief.next_batch.batch_id == "saudi_005"
    ...
    assert "validate_dataset.py data/corpus/chat/jsonl/dialogue_batch_v2_saudi_005.jsonl" in brief.after_export_commands[0]
...
def test_phase22_next_batch_endpoint() -> None:
    ...
    assert body["next_batch"]["batch_id"] == "saudi_005"
    assert body["next_batch"]["dialect"] == "saudi"
    assert body["next_batch"]["target_records"] == 25
    assert len(body["suggested_topics"]) >= 6
    assert body["warnings"]
    assert "validate_dataset.py data/corpus/chat/jsonl/dialogue_batch_v2_saudi_005.jsonl" in body["after_export_commands"][0]
...
def test_phase22_completion_gate_blocks_advancement_until_complete() -> None:
    ...
    assert gate.training_records == 330
    assert gate.target_records == 500
    assert gate.remaining_records == 170
    assert gate.current_next_batch == "saudi_005"
    ...
    assert "complete_next_batch:saudi_005" in gate.missing_requirements
...
def test_phase22_completion_gate_endpoint() -> None:
    ...
    assert body["current_next_batch"] == "saudi_005"
    assert body["completion_checks"]["corpus_target_met"] is False
    assert "complete_next_batch:saudi_005" in body["missing_requirements"]
...
def test_phase22_collection_plan_cli_is_read_only() -> None:
    ...
    assert "estimated_batches             : 4" in proc.stdout
    assert "planned batches:" in proc.stdout
    assert "#01 saudi_005 (saudi, 50 records, minimum_saudi)" in proc.stdout
    assert "synthetic_llm_data_allowed    : false" in proc.stdout
...
def test_phase22_next_batch_cli_is_read_only() -> None:
    ...
    assert "next_batch                    : saudi_005" in proc.stdout
...
def test_phase22_completion_gate_cli_is_read_only() -> None:
    ...
    assert "current_next_batch            : saudi_005" in proc.stdout
