...
def test_phase22_readiness_reports_current_gap() -> None:
    ...
    assert decision.remaining_records == 25
    assert decision.dialect_counts == {"msa": 237, "saudi": 238}
...
def test_phase22_endpoint() -> None:
    ...
    assert body["training_records"] == 475
...
def test_phase22_collection_plan_calculates_real_quotas() -> None:
    ...
    assert plan.current_records == 475
    assert plan.remaining_records == 25
    assert plan.batch_size == 25
    assert plan.estimated_batches == 1
    assert plan.quota_by_dialect == {}
    assert plan.flexible_records_after_minimums == 25
    ...
    assert len(plan.planned_batches) == 1
    assert plan.planned_batches[0].batch_id == "flex_004"
...
def test_phase22_collection_plan_endpoint() -> None:
    ...
    assert body["planned_batches"][0]["batch_id"] == "flex_004"
    assert body["planned_batches"][0]["suggested_output_path"].endswith("dialogue_batch_v2_flex_004.jsonl")
    assert body["planned_batches"][-1]["batch_id"] == "flex_004"
...
def test_phase22_next_batch_brief_points_to_next_saudi_batch() -> None:
    ...
    assert brief.next_batch.batch_id == "flex_004"
    ...
    assert "validate_dataset.py data/corpus/chat/jsonl/dialogue_batch_v2_flex_004.jsonl" in brief.after_export_commands[0]
...
def test_phase22_next_batch_endpoint() -> None:
    ...
    assert body["next_batch"]["batch_id"] == "flex_004"
    ...
    assert "validate_dataset.py data/corpus/chat/jsonl/dialogue_batch_v2_flex_004.jsonl" in body["after_export_commands"][0]
...
def test_phase22_completion_gate_blocks_advancement_until_complete() -> None:
    ...
    assert gate.training_records == 475
    assert gate.target_records == 500
    assert gate.remaining_records == 25
    assert gate.current_next_batch == "flex_004"
    ...
    assert "complete_next_batch:flex_004" in gate.missing_requirements
...
def test_phase22_completion_gate_endpoint() -> None:
    ...
    assert body["current_next_batch"] == "flex_004"
    assert body["completion_checks"]["corpus_target_met"] is False
    assert "complete_next_batch:flex_004" in body["missing_requirements"]
...
def test_phase22_collection_plan_cli_is_read_only() -> None:
    ...
    assert "#01 flex_004 (msa_or_saudi, 25 records, flexible_after_minimums)" in proc.stdout
    assert "synthetic_llm_data_allowed    : false" in proc.stdout
...
def test_phase22_next_batch_cli_is_read_only() -> None:
    ...
    assert "next_batch                    : flex_004" in proc.stdout
...
def test_phase22_completion_gate_cli_is_read_only() -> None:
    ...
    assert "current_next_batch            : flex_004" in proc.stdout
