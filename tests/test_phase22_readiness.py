...
def test_phase22_readiness_reports_current_gap() -> None:
    ...
    assert decision.training_records == 227
    assert decision.target_records == 500
    assert decision.remaining_records == 273
    assert decision.dialect_counts == {"msa": 197, "saudi": 30}
...
def test_phase22_readiness_requires_balance_before_phase23() -> None:
    ...
    assert decision.dialect_shortfalls["msa"] == 3
...
def test_phase22_endpoint() -> None:
    ...
    assert body["training_records"] == 227
...
def test_phase22_collection_plan_calculates_real_quotas() -> None:
    ...
    assert plan.current_records == 227
    assert plan.remaining_records == 273
    assert plan.batch_size == 25
    assert plan.estimated_batches == 11
    assert plan.quota_by_dialect == {"msa": 3, "saudi": 170}
    ...
    assert len(plan.planned_batches) == 12
    assert plan.planned_batches[0].batch_id == "msa_008"
    assert plan.planned_batches[0].dialect == "msa"
    assert plan.planned_batches[0].target_records == 3
    assert plan.planned_batches[1].batch_id == "saudi_001"
    assert plan.planned_batches[7].batch_id == "saudi_007"
    assert plan.planned_batches[7].target_records == 20
...
def test_phase22_collection_plan_endpoint() -> None:
    ...
    assert body["quota_by_dialect"] == {"msa": 3, "saudi": 170}
    assert body["synthetic_llm_data_allowed"] is False
    assert len(body["planned_batches"]) == 7
    assert body["planned_batches"][0]["batch_id"] == "msa_008"
    assert body["planned_batches"][0]["suggested_output_path"].endswith("dialogue_batch_v2_msa_008.jsonl")
    assert body["planned_batches"][-1]["batch_id"] == "flex_002"


def test_phase22_next_batch_brief_points_to_next_msa_batch() -> None:
    brief = build_phase22_next_batch_brief()
    assert brief.status == "AUTHOR_NEXT_REVIEW_BATCH"
    assert brief.next_batch is not None
    assert brief.next_batch.batch_id == "msa_008"
    assert brief.next_batch.dialect == "msa"
    assert brief.next_batch.target_records == 3
    ...
    assert "validate_dataset.py data/corpus/chat/jsonl/dialogue_batch_v2_msa_008.jsonl" in brief.after_export_commands[0]
...
def test_phase22_next_batch_endpoint() -> None:
    ...
    assert body["next_batch"]["batch_id"] == "msa_008"
    assert body["next_batch"]["dialect"] == "msa"
    assert body["next_batch"]["target_records"] == 3
    assert len(body["suggested_topics"]) >= 80
    assert body["warnings"]
    assert "validate_dataset.py data/corpus/chat/jsonl/dialogue_batch_v2_msa_008.jsonl" in body["after_export_commands"][0]
...
def test_phase22_completion_gate_blocks_advancement_until_complete() -> None:
    ...
    assert gate.training_records == 227
    assert gate.target_records == 500
    assert gate.remaining_records == 273
    assert gate.current_next_batch == "msa_008"
    ...
    assert "complete_next_batch:msa_008" in gate.missing_requirements
...
def test_phase22_completion_gate_endpoint() -> None:
    ...
    assert body["current_next_batch"] == "msa_008"
    assert body["completion_checks"]["corpus_target_met"] is False
    assert "complete_next_batch:msa_008" in body["missing_requirements"]
...
def test_phase22_collection_plan_cli_is_read_only() -> None:
    ...
    assert "#01 msa_008 (msa, 3 records, minimum_msa)" in proc.stdout
    assert "synthetic_llm_data_allowed    : false" in proc.stdout
...
def test_phase22_next_batch_cli_is_read_only() -> None:
    ...
    assert "next_batch                    : msa_008" in proc.stdout
...
def test_phase22_completion_gate_cli_is_read_only() -> None:
    ...
    assert "current_next_batch            : msa_008" in proc.stdout
