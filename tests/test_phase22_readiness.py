...
def test_phase22_readiness_reports_current_gap() -> None:
    ...
    assert decision.remaining_records == 420
    assert decision.dialect_counts == {"msa": 50, "saudi": 30}
...
def test_phase22_readiness_requires_balance_before_phase23() -> None:
    ...
    assert decision.dialect_shortfalls["msa"] == 150
...
def test_phase22_endpoint() -> None:
    ...
    assert body["training_records"] == 80
...
def test_phase22_collection_plan_calculates_real_quotas() -> None:
    ...
    assert plan.current_records == 80
    assert plan.remaining_records == 420
    assert plan.batch_size == 25
    assert plan.estimated_batches == 17
    assert plan.quota_by_dialect == {"msa": 150, "saudi": 170}
    ...
    assert len(plan.planned_batches) == 17
    assert plan.planned_batches[0].batch_id == "msa_003"
    assert plan.planned_batches[0].dialect == "msa"
    assert plan.planned_batches[0].target_records == 25
    assert plan.planned_batches[5].batch_id == "msa_008"
    assert plan.planned_batches[6].batch_id == "saudi_001"
    assert plan.planned_batches[12].batch_id == "saudi_007"
    assert plan.planned_batches[12].target_records == 20
...
def test_phase22_collection_plan_endpoint() -> None:
    ...
    assert body["estimated_batches"] == 9
    assert body["quota_by_dialect"] == {"msa": 150, "saudi": 170}
    assert body["synthetic_llm_data_allowed"] is False
    assert len(body["planned_batches"]) == 9
    assert body["planned_batches"][0]["batch_id"] == "msa_003"
    assert body["planned_batches"][0]["suggested_output_path"].endswith("dialogue_batch_v2_msa_003.jsonl")
...
def test_phase22_next_batch_brief_points_to_next_msa_batch() -> None:
    ...
    assert brief.next_batch.batch_id == "msa_003"
    ...
    assert "validate_dataset.py data/corpus/chat/jsonl/dialogue_batch_v2_msa_003.jsonl" in brief.after_export_commands[0]
...
def test_phase22_next_batch_endpoint() -> None:
    ...
    assert body["next_batch"]["batch_id"] == "msa_003"
    assert body["next_batch"]["dialect"] == "msa"
    assert body["next_batch"]["target_records"] == 25
    assert len(body["suggested_topics"]) >= 80
    assert body["warnings"]
    assert "validate_dataset.py data/corpus/chat/jsonl/dialogue_batch_v2_msa_003.jsonl" in body["after_export_commands"][0]
...
def test_phase22_completion_gate_blocks_advancement_until_complete() -> None:
    ...
    assert gate.training_records == 80
    assert gate.target_records == 500
    assert gate.remaining_records == 420
    assert gate.current_next_batch == "msa_003"
    ...
    assert "complete_next_batch:msa_003" in gate.missing_requirements
...
def test_phase22_completion_gate_endpoint() -> None:
    ...
    assert body["current_next_batch"] == "msa_003"
    assert body["completion_checks"]["corpus_target_met"] is False
    assert "complete_next_batch:msa_003" in body["missing_requirements"]
...
def test_phase22_collection_plan_cli_is_read_only() -> None:
    ...
    assert "estimated_batches             : 9" in proc.stdout
    assert "planned batches:" in proc.stdout
    assert "#01 msa_003 (msa, 50 records, minimum_msa)" in proc.stdout
    assert "synthetic_llm_data_allowed    : false" in proc.stdout
...
def test_phase22_next_batch_cli_is_read_only() -> None:
    ...
    assert "next_batch                    : msa_003" in proc.stdout
...
def test_phase22_completion_gate_cli_is_read_only() -> None:
    ...
    assert "current_next_batch            : msa_003" in proc.stdout
