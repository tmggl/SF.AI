...
- Phase 26 اكتمل: `make phase26-readiness` و`GET /system/phase26-readiness`
  قررا أن `SF-50M` غير جاهز الآن: corpus `500/500` ممتاز كبداية لكنه دون
  حد `5000` العملي، وruntime quality/hallucination/repetition gates لم تمر.
- Phase 27 اكتمل: `make phase27-dialogue-eval` مرّر `19/19` turn في suite
  متعدد الأدوار، ووضع خطة توسعة دقيقة إلى `5000`.
- بدأ تنفيذ توسعة Phase 27: أضيفت أول دفعتين `v3_msa_001` و`v3_saudi_001`
  بإجمالي `50` سجلًا، فأصبح corpus الحالي `550` (`msa=275`, `saudi=275`)
  والمتبقي إلى `5000` هو `4450`.
...
- `chat/jsonl`: يحتوي الآن 550 سجل حوار محكوم. أول 500 استُخدمت لتدريب tokenizer v2، ثم بدأت توسعة Phase 27 بعد ذلك.
