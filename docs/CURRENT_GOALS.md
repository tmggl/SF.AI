...
- Phase 26 اكتمل: `make phase26-readiness` و`GET /system/phase26-readiness`
  ...
  سياسة منع الحوارات التشغيلية، ثم صار `3643` سجلًا طبيعيًا فقط
  (`msa=1799`, `saudi=1844`) بعد حذف `907` سجلًا ممنوعًا، وهو لا يزال
  دون حد `5000` العملي، وruntime quality/hallucination/repetition gates
  لم تمر.
...
- بدأ تنفيذ توسعة Phase 27: أضيفت Batches 001/002/003 ثم طُبقت سياسة
  `training_forbidden_operational_internal_dialogue`. الحالة الحالية بعد
  التنظيف: `3643` سجلًا تدريبيًا طبيعيًا (`msa=1799`, `saudi=1844`)،
  والمتبقي إلى `5000` هو `1357`.
...
- `chat/jsonl`: يحتوي الآن 3643 سجل حوار محكوم. أول 500 استُخدمت لتدريب tokenizer v2، ثم بدأت توسعة Phase 27 بعد ذلك.
