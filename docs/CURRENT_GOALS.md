...
تنفيذ توسعة corpus المحكومة بعد اكتمال Phase 27 كـ Dialogue Evaluation v2
وخطة بيانات. Phase 27 أثبتت أن التوجيه الحالي يمر baseline متعدد الأدوار،
لكن الردود ما زالت `template` وليست مولدًا مفتوحًا.
...
- Phase 26 اكتمل: `make phase26-readiness` و`GET /system/phase26-readiness`
  قررا أن `SF-50M` غير جاهز الآن: corpus بدأ من `500/500` ثم وصل بعد
  Batch 003 إلى `1550`، لكنه لا يزال دون حد `5000` العملي،
  وruntime quality/hallucination/repetition gates لم تمر.
- Phase 27 اكتمل: `make phase27-dialogue-eval` مرّر `19/19` turn في suite
  متعدد الأدوار، ووضع خطة توسعة دقيقة إلى `5000`.
- بدأ تنفيذ توسعة Phase 27: أضيف Batch 001 بإجمالي `50` سجلًا، ثم Batch 002 وBatch 003
  الكبيرتان بإجمالي `1000` سجل، فأصبح corpus الحالي `1550`
  (`msa=775`, `saudi=775`) والمتبقي إلى `5000` هو `3450`.
...
- `chat/jsonl`: يحتوي الآن 1550 سجل حوار محكوم. أول 500 استُخدمت لتدريب tokenizer v2، ثم بدأت توسعة Phase 27 بعد ذلك.
...
- **Phase 27:** تقييم حوار متعدد الأدوار + خطة توسيع corpus — مكتمل مع توسعة مطلوبة.
...
- أول فرصة لحوار قصير مولّد يقنعك: بعد تنفيذ خطة Phase 27، ثم إعادة بوابة SF-50M، ثم تدريب `SF-50M` ومروره في eval v2.
