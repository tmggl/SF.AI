...
- Phase 26 اكتمل: `make phase26-readiness` و`GET /system/phase26-readiness`
  ...
  سياسة منع الحوارات التشغيلية، ثم صار `643` سجلًا طبيعيًا فقط
  (`msa=299`, `saudi=344`) بعد حذف `907` سجلًا ممنوعًا، وهو لا يزال
...
- بدأ تنفيذ توسعة Phase 27: أضيفت Batches 001/002/003 ثم طُبقت سياسة
  ...
  التنظيف: `643` سجلًا تدريبيًا طبيعيًا (`msa=299`, `saudi=344`)،
  والمتبقي إلى `5000` هو `4357`.
...
- `chat/jsonl`: يحتوي الآن 643 سجل حوار محكوم. أول 500 استُخدمت لتدريب tokenizer v2، ثم بدأت توسعة Phase 27 بعد ذلك.
