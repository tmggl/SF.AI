...
- Phase 26 اكتمل: `make phase26-readiness` و`GET /system/phase26-readiness`
  ...
  Batch 002 إلى `1050`، لكنه لا يزال دون حد `5000` العملي،
...
- بدأ تنفيذ توسعة Phase 27: أضيف Batch 001 بإجمالي `50` سجلًا، ثم Batch 002
  الكبيرة بإجمالي `500` سجل، فأصبح corpus الحالي `1050`
  (`msa=525`, `saudi=525`) والمتبقي إلى `5000` هو `3950`.
...
- `chat/jsonl`: يحتوي الآن 1050 سجل حوار محكوم. أول 500 استُخدمت لتدريب tokenizer v2، ثم بدأت توسعة Phase 27 بعد ذلك.
