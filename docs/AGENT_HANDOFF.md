...
```
راجع `docs/PHASE_STATUS.md` لآخر رقم اختبارات موثق بعد Phase 27.41.
```
...
تدريب tokenizer v1 اكتمل في Phase 12. Smoke LM training اكتمل في Phase 13، وSF-10M v0.1 المحدود اكتمل في Phase 14، لكنه خام ومكرر وغير جاهز لاختبار سامي كمولد حواري. Phase 15 أضاف `NativeGenerator` و`GenerationPolicy` وmetadata يوضح هل الرد `template` أو `sf_10m_v0_1`، لكنه لم يجعل المولد مقنعًا. Phase 16 أضاف prompt suites وeval report، ثم فُتح مختبر سامي المحلي للقياس والتطوير. Phase 17 أضاف `ChatRagBridge` و`ContextBuilder` كربط محلي اختياري مع `HybridRetriever`. Phase 18 أضاف دورة تحسين بيانات محكومة من واجهة الشات. Phase 19 أضاف بوابة جاهزية SF-50M. Phase 20 أضاف بوابات تفعيل المجالات، ولا يفعّل أي skeleton تلقائيًا. Phase 21 ثبت خارطة Phase 22–30 للوصول إلى حوار مولّد مقنع. Phase 22 أضاف وأكمل بوابة Gold Dialogue Corpus v2: اكتمل عند 500/500 (`msa=250`, `saudi=250`). Phase 23 درّب tokenizer v2: `vocab=4493`, `merges=4386`. Phase 24–27.30 نقلت المولد عبر تجارب training/canary/repair حتى fresh mixed `16/18`. Phase 27.31–27.33 مررت كل بوابات التوليد المحلية بلا prompt leakage. Phase 27.34–27.43 فتحت تجربة مولد محروسة ثم حسّنت الحارس والتوسعة حتى فشل مرشح weak-lane الجزئي `10/16`. Phase 27.44 أنشأت tokenizer v6 وحسنت weak-lane إلى `6/6` لكن الإجمالي `11/16`. Phase 27.45 تراجعت `9/16`. Phase 27.46 وصلت إلى `14/16`. Phase 27.47 أصلحت conditioning لموضوعي `الوفاء/الشجاعة` ومرّت offline `16/16`. Phase 27.48 فتحت `sf_10m_phase27_47` داخل `generator_trial=true` فقط ومرّت live API `19/19`. Phase 27.49 وسعت live probes إلى `33/33` وأصلحت كشف النصيحة السعودية `وش تنصحني اسوي`. Phase 27.50 ألغت القوالب من `/chat/message` والواجهة: المولد أو رد فارغ، gate `7/7`. Phase 27.51 اختبرت الحوار الطبيعي بلا intent/topic conditioning وفشلت بوضوح (`live=3/22`, `raw natural=1/20`). Phase 27.52 ضاعفت تدريب `SF-10M` إلى `9200` خطوة، وتحسن raw natural إلى `5/20` فقط. Phase 27.53 وسعت البيانات إلى `10,540` زوجًا فريدًا و`18,000` خطوة، لكن raw natural كان `2/36` مع خلط/fragments؛ checkpoint الجديد لا يفتح في runtime. Phase 27.54 منعت التكبير الكامل. Phase 27.55 قارنت `SF-10M` و`SF-50M`: `3/20` مقابل `4/20`. Phase 27.56 شخّصت objective/eval/tokenizer. Phase 27.57 ثبتت حزمة الإصلاح: `18` عبارة محمية، تغطية `9/9`, semantic alignment، وresponse families. Phase 27.58 دربت tokenizer v7 مع `53` protected terms/phrases ومررت حماية 27.57 كقطعة واحدة، لكن bounded alignment probe فشل `4/15`. Phase 27.59 أصلحت alignment المحدود ومرّت `15/15` على tokenizer v7. Phase 27.60 اختبرت التعميم الطبيعي الأوسع وفشلت `12/30`. Phase 27.61 حسّنت النتيجة إلى `18/30` ونجحت planning/support. Phase 27.62 تراجعت إلى `10/30` بسبب ترتيب curriculum الكتلي. Phase 27.63 أصلحت الترتيب إلى interleaved ورفعت canary إلى `26/30`. Phase 27.64 أثبتت أن tokenizer v8 مطلوب لأن `التعاون/الاحترام` تراجعا من single-piece في v6 إلى قطع متعددة في v7. Phase 27.65 دربت tokenizer v8 فقط ومررت topic probe `8/8`. Phase 27.66 دربت LM repair محدودًا على tokenizer v8 ومررت broader canary `30/30`. Phase 27.67 اختبرت fresh shadow canary وفشلت `30/50`. Phase 27.68 أصلحت الفشل المعروف إلى `50/50` وحافظت على regression `30/30`. Phase 27.69 اختبرت fresh shadow جديدًا ونجحت جزئيًا `56/60` مع open_social `8/12`. Phase 27.70 جرّبت إصلاح open_social وfine-tune سيادي من checkpoint 27.68 لكنها لم تتجاوز baseline (`55/60`, `48/50`, `30/30`). Phase 27.71 قارنت مرشحي v8 واختارت `phase27_68` كأفضل مرشح بنتيجة `136/140`، لكنه لم يمر كل البوابات. Phase 27.72 نفذت micro-repair من هذا المرشح ورفعت النتيجة إلى `138/140` مع known `50/50` وregression `30/30`، وبقي فشلان open_social. Phase 27.73 فحصت الفشلين: سدّت فجوة guard لشظايا مثل `بمها`، وأثبتت أن الفشل المتبقي semantic collapse إلى تعريف موضوع. Phase 27.74 جرّبت إصلاحًا ضيقًا من checkpoint 27.72، لكن أفضل مرشح تراجع إلى `56/60` fresh و`49/50` known و`30/30` regression بسبب fragments مثل `بس الفة` و`موضوعاموضوععن`. Phase 27.75 فحصت الاستراتيجية وأثبتت أن tokenizer v8 يفك `بسالفة` إلى `بس الفة`؛ أضيفت `resources/tokenization/protected_phrases_phase27_75.txt` لقواعد tokenization. Phase 27.76 درّبت tokenizer v9 فقط ومرّت open_social boundary probe (`17/17` roundtrip و`15/15` protected pack single-piece). Phase 27.77 درّبت LM محدودًا من الصفر على tokenizer v9، لكنه فشل كتوليد (`54/60` fresh، `45/50` known، `30/30` regression) بسبب خلط عائلات الردود. الوضع الحالي `5943` (`msa=2949`, `saudi=2994`)؛ التالي Phase 27.78 inspect Phase 27.77 failures and revise v9 LM strategy، ولا runtime switch.
