...
تم تشغيل Phase 12 tokenizer v1 بإذن صريح من سامي على corpus صغير سابقًا. لا تعامل v1 كتشغيل لغوي متوازن؛ Phase 22 رفع corpus الحالي إلى 400 سجلًا فقط (`msa=200`, `saudi=200`). التوازن الأساسي اكتمل، لكن العدد الكلي ما زال غير كافٍ.
...
- السبب: corpus الحالي 400 سجلًا فقط، والحد الأدنى العملي الحالي 5000 سجل محكوم مع توازن `msa + saudi`.
...
- الموجود: 400 سجلًا: 200 `msa` و200 `saudi`
- الهدف: 500 سجل، مع 200 على الأقل لكل من `msa` و`saudi`
- خطة الجمع الحالية: 100 مرنة، نحو 4 batches بحجم 25
- المهمة الفورية الحالية: `flex_001` عبر `make phase22-next-batch`
...
- أكمل الدفعات المرنة `flex_001..flex_004` حتى تمر Phase 22 readiness، ثم أعد corpus-audit وPhase 19/20/22 gates.
