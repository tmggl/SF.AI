...
```
راجع `docs/PHASE_STATUS.md` لآخر رقم اختبارات موثق بعد Phase 27.30.
```
...
تدريب tokenizer v1 اكتمل في Phase 12. Smoke LM training اكتمل في Phase 13، وSF-10M v0.1 المحدود اكتمل في Phase 14، لكنه خام ومكرر وغير جاهز لاختبار سامي كمولد حواري. Phase 15 أضاف `NativeGenerator` و`GenerationPolicy` وmetadata يوضح هل الرد `template` أو `sf_10m_v0_1`، لكنه لم يجعل المولد مقنعًا. Phase 16 أضاف prompt suites وeval report، ثم فُتح مختبر سامي المحلي للقياس والتطوير. Phase 17 أضاف `ChatRagBridge` و`ContextBuilder` كربط محلي اختياري مع `HybridRetriever`. Phase 18 أضاف دورة تحسين بيانات محكومة من واجهة الشات. Phase 19 أضاف بوابة جاهزية SF-50M. Phase 20 أضاف بوابات تفعيل المجالات، ولا يفعّل أي skeleton تلقائيًا. Phase 21 ثبت خارطة Phase 22–30 للوصول إلى حوار مولّد مقنع. Phase 22 أضاف وأكمل بوابة Gold Dialogue Corpus v2: اكتمل عند 500/500 (`msa=250`, `saudi=250`). Phase 23 درّب tokenizer v2: `vocab=4493`, `merges=4386`. Phase 24 درّب `SF-10M v0.2` 2000 خطوة، وحقق eval loss `2.5779` وperplexity `13.17`، لكنه ما زال غير متماسك ولذلك runtime محظور. Phase 25 أضاف canary guard وحجب تجربة v0.2 الحقيقية بـ `generation_guard:malformed_token`. Phase 26 أضاف بوابة readiness ورفض تدريب `SF-50M` الآن رغم نجاح corpus gate، لأن runtime quality لم تنجح. Phase 27 أضاف eval v2 متعدد الأدوار وخطة corpus: `19/19` baseline، لكن كل الردود `template`. بدأت التوسعة بعد ذلك بإضافة Batch 001 ثم Batch 002 ثم Batch 003، ثم تنظيف الحوارات التشغيلية، ثم Batch 004 وBatch 005 وBatch 006 الطبيعية بحجم `1500` لكل دفعة. Phase 27.5/27.6 جربتا تدريب SF-10M بصيغة الحوار وassistant-target وبقي runtime محظورًا. Phase 27.7 أضاف split ثابتًا (`train=4703`, `eval=540`)، و100 gold social، وcanary prompt-aware. Phase 27.8 درّب `SF-10M v0.6` على train split وقاس eval split: أفضل loss `5.0227`/ppl `151.82`، لكن canary حجب 10/10 عينات بسبب fragments. Phase 27.9 أضاف generation quality harness: `make phase27-generation-quality` يثبت `0/10` passed و`model_artifact_fragment`. Phase 27.10 أضاف `300` gold short-response ودرب `SF-10M v0.7`: أفضل eval `4.7512`/ppl `115.72`، لكن generation-quality بقي `0/10`. Phase 27.11 شغّل gold overfit probe على `16` ردًا؛ loss صار شبه صفري لكن clean-stop فشل `0/16`. Phase 27.12 أضاف assistant `<eos>` وdialect conditioning؛ probe صار `5/16` تطابق كامل و`9/16` بلا فشل guard. Phase 27.13 درّب `SF-10M v0.8` على split التدريب، وأفضل eval صار loss `3.1875`/ppl `24.23`، لكن generation-quality الصارم بقي `3/10` بسبب fragments ومطابقة اجتماعية ضعيفة. Phase 27.14 ثبّت Sovereign Training Quality Tooling. Phase 27.15 أضاف 400 عينة gold وno-repeat decoding ودرب `SF-10M v0.10`: أفضل eval loss `3.0452`/ppl `21.01`، لكن canary الدلالي الصارم بقي `0/10`. Phase 27.16 أضاف sample-isolated packing ودرب `SF-10M v0.11`: أفضل eval loss `4.0573`/ppl `57.82` وcanary بقي محجوبًا (`2/10` في step2000 و`0/10` في step6000). Phase 27.17 شغّل prompt-answer micro-probe على `32` زوجًا وحقق `27/32` ردًا صحيحًا، لكن runtime بقي محجوبًا بسبب كسور لفظية/حروفية. Phase 27.18 أضاف hygiene audit وحدد 5 عبارات تتجزأ بقوة، مع حجب كل الكسور المرصودة في `GenerationGuard`. Phase 27.19 اختبر repair examples مركزًا على هذه العبارات، لكن النتيجة بقيت `27/32`؛ أمثلة repair وحدها لا تكفي. Phase 27.20 أضاف دعم protected phrases داخل tokenizer. Phase 27.21 درّب tokenizer v3 ونجح في جعل العبارات `max_pieces=1`، لكن micro-probe فشل `25/32` بسبب spacing/boundary مثل `سواونخفف`. Phase 27.22 أصلح decode boundary ورفع النتيجة إلى `29/32` مع `glued_left=0`. Phase 27.23 أضاف semantic/lexical repair متوازن ورفع micro-probe إلى `30/32`. Phase 27.24 أضاف tokenizer minimal `v4_min_lexical` ووصل إلى `32/32` في micro-probe. Phase 27.25 اختبر نفس checkpoint على 16 سؤالًا held-out وفشل `8/16`. Phase 27.26–27.30 أضافت repair series وintent/topic conditioning ورفعت fresh mixed shadow إلى `16/18`، لكن runtime بقي محجوبًا. الوضع الحالي `5943` (`msa=2949`, `saudi=2994`) والتالي Phase 27.31 broader natural intent/topic dataset قبل أي runtime أو `SF-50M`.
