...
تم تشغيل Phase 12 tokenizer v1 بإذن صريح من سامي على corpus صغير سابقًا. لا تعامل v1 كتشغيل لغوي متوازن؛ Phase 22 رفع corpus الحالي إلى 380 سجلًا فقط (`msa=200`, `saudi=180`) وما زال التوازن والعدد غير كافيين.
...
- السبب: corpus الحالي 380 سجلًا فقط، والحد الأدنى العملي الحالي 5000 سجل محكوم مع توازن `msa + saudi`.
...
- الموجود: 380 سجلًا: 200 `msa` و180 `saudi`
- الهدف: 500 سجل، مع 200 على الأقل لكل من `msa` و`saudi`
- خطة الجمع الحالية: 20 سعودي + 100 مرنة، نحو 5 batches بحجم 25
- المهمة الفورية الحالية: `saudi_007` عبر `make phase22-next-batch`
...
- أكمل دفعة `saudi_007` ثم المرنة حتى تمر Phase 22 readiness، ثم أعد corpus-audit وPhase 19/20/22 gates.
