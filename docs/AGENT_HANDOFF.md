...
تم تشغيل Phase 12 tokenizer v1 بإذن صريح من سامي على corpus صغير سابقًا. لا تعامل v1 كتشغيل لغوي متوازن؛ Phase 22 رفع corpus الحالي إلى 80 سجلًا فقط (`msa=50`, `saudi=30`) وما زال التوازن والعدد غير كافيين.
...
- السبب: corpus الحالي 80 سجلًا فقط، والحد الأدنى العملي الحالي 5000 سجل محكوم مع توازن `msa + saudi`.
...
- الموجود: 80 سجلًا: 50 `msa` و30 `saudi`
- الهدف: 500 سجل، مع 200 على الأقل لكل من `msa` و`saudi`
- خطة الجمع الحالية: 150 فصحى + 170 سعودي + 100 مرنة، نحو 17 batch بحجم 25
- المهمة الفورية الحالية: `msa_003` عبر `make phase22-next-batch`
- أضيفت دفعتان فصيحتان معتمدتان: `data/corpus/chat/jsonl/dialogue_batch_v2_msa_001.jsonl` و`dialogue_batch_v2_msa_002.jsonl` مع بطاقات provenance.
...
- أكمل دفعات `msa_003..msa_008` ثم `saudi_001..saudi_007` ثم المرنة حتى تمر Phase 22 readiness، ثم أعد corpus-audit وPhase 19/20/22 gates.
