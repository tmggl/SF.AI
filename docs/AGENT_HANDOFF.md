...
تم تشغيل Phase 12 tokenizer v1 بإذن صريح من سامي على corpus صغير سابقًا. لا تعامل v1 كتشغيل لغوي متوازن؛ Phase 22 رفع corpus الحالي إلى 355 سجلًا فقط (`msa=200`, `saudi=155`) وما زال التوازن والعدد غير كافيين.
...
- السبب: corpus الحالي 355 سجلًا فقط، والحد الأدنى العملي الحالي 5000 سجل محكوم مع توازن `msa + saudi`.
...
- الموجود: 355 سجلًا: 200 `msa` و155 `saudi`
- الهدف: 500 سجل، مع 200 على الأقل لكل من `msa` و`saudi`
- خطة الجمع الحالية: 45 سعودي + 100 مرنة، نحو 6 batches بحجم 25
- المهمة الفورية الحالية: `saudi_006` عبر `make phase22-next-batch`
...
- أكمل دفعات `saudi_006..saudi_007` ثم المرنة حتى تمر Phase 22 readiness، ثم أعد corpus-audit وPhase 19/20/22 gates.
