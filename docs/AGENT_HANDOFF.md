...
`PHASE27_126_SINALAB_SYNONYMS_REFERENCE_RUNTIME_POLICY_DESIGN_DECISION`. مرّت بوابات
...
checksum/schema dry-run. Phase 27.116 نزّل artifact في quarantine محلي
git-ignored، وسجل checksum/schema دون حفظ raw rows أو كتابة corpus. Phase 27.117
فحص sample quality/dedupe بأرقام فقط دون نشر raw terms. Phase 27.118 صمم
...
Phase 27.121 بنى records محلية gitignored: `1093` reference records و`685`
eval candidates؛ المرفوع counts/hashes فقط بلا raw terms.
Phase 27.122 اختبر query/eval محليًا في الذاكرة: lookup `685/685`,
duplicate keys `0`, ولا runtime activation.
Phase 27.123 صمم عقد adapter مرجعيًا فقط: `SinaLabSynonymsReferenceAdapter`,
redaction policy، و`max_results=5/10`، مع منع raw terms/query rows/runtime/chat/corpus/tokenizer/training.
Phase 27.124 كتب skeleton adapter واختبره بسجلات synthetic فقط؛ redaction/hash/quality
contract مرّت، ولا runtime ولا ChatModule ولا corpus/tokenizer/training.
Phase 27.125 شغل adapter محليًا على reference layer gitignored: lookup `685/685`,
redaction `685/685`, term leak `0`، بلا runtime ولا ChatModule ولا corpus/tokenizer/training.
Phase 27.126 صمم runtime policy بلا تفعيل: default disabled، aggregate-only،
no raw terms/query rows/logs، وno template masking.
