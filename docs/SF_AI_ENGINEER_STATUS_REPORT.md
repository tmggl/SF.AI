...
**تاريخ التقرير:** 2026-05-23  
**الحالة الحالية:** Phase 27.19 / 30  
**المسار اللغوي:** العربية الفصحى + اللهجة السعودية فقط  
**القاموس المرجعي:** Saudi Seed v1 + `safety_terms.yaml`  
**قرار runtime الحالي:** المولد السيادي غير مفعّل للمحادثة الحرة؛ الواجهة تستخدم routing/templates مع حراسة واضحة.
...
وصل المشروع إلى مرحلة متقدمة من البنية، الحوكمة، جمع corpus سيادي، تدريب tokenizer من الصفر، تدريب عدة نسخ صغيرة من نموذج `SF-10M`، بناء واجهة محادثة، بناء canary/eval/guards، وتثبيت استراتيجية تكبير تدريجي. لكنه لم يصل بعد إلى حوار مولّد مقنع داخل الواجهة؛ السبب الحالي ليس غياب التدريب، بل أن النموذج الصغير يتعلم جزئيًا ثم ينتج كسورًا لغوية وتكرارًا أو ردودًا غير مرتبطة كفاية بالسؤال.
...
- لا نفتح Phase 28.
- نكمل Phase 27.20: إصلاح tokenizer/protected phrases قبل تدريب واسع.
...
1. إصلاح جودة التوليد القصير في `SF-10M`.
2. منع الكسور مثل: `المعنى/وأين/يستخدم؟` والخلط الحرفي.
3. إثبات clean-stop وsemantic correctness على canary صغير.
4. بعد ذلك فقط إعادة بوابة `SF-50M`.
...
هذا مهم: **الواجهة الحالية ليست دليلًا على نجاح المولد**. هي واجهة تشغيل آمنة تعرض الرد الجاهز أو المحروس، وتمنع المولد الضعيف من تضليل المستخدم.
