...
بعد Phase 27.95، ثبت أن التدريب المحدود وحده لم يكفِ: best checkpoint
...
بعد Phase 27.103، أصبحت حزمة topic prototype contrastive جاهزة ومنضبطة
...
المسار الحالي أُعيد تثبيته عند Phase 27.79 كخطة
Objective/Curriculum/Decoding. لا تدريب جديد قبل أن تمر بوابات 27.80:
objective renderer، assistant-only loss mask، stratified round-robin
curriculum، guarded decoding، contrastive eval، checkpoint selector،
held-out canary، corpus-audit، sensitive scan، full tests، وMPS/AMP smoke.
...
### 11.1 SF-native Objective/Curriculum/Decoding Acceleration Track

هذا المسار أصبح الحاكم الحالي بعد Phase 27.104:

- لا Qwen ولا Llama ولا Mistral ولا Gemma ولا أي pretrained/open-weight.
- لا training عشوائي ولا retrain كامل لكل مشكلة صغيرة.
- لا تكبير إلى `SF-50M` قبل إثبات أن capacity هي السبب الأكبر.
- كل عينة تدريب مستقبلية في هذا المسار تستخدم سياقًا صريحًا:
...
- أسطر النطاق والعائلة والمستخدم تدخل النموذج كسياق.
- loss يحسب على رد المساعد و`<eos>` فقط.
- curriculum يجب أن يكون stratified round-robin بين:
  `open_social`, `followup`, `planning`, `support`, `topic`.
- decoding يجب أن يطبق `stop_at_eos`, no-repeat ngram,
  repetition penalty, topic substitution guard, family drift guard.
- إذا فشل الرد، يرجع النظام `blocked` metadata؛ لا قالب يخفي الفشل.
- LoRA/QLoRA مؤجلة ومسموحة فقط فوق نماذج SF-native.
