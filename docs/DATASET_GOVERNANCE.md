...
- `owner_user_id` إلزامي: مالك الحوار.
- `created_by_user_id` إلزامي: من أنشأ/صدّر/راجع السجل.
- `target_user_id` إلزامي: المستخدم الذي يجوز أن تعود له هذه البيانات لاحقًا.
- `user_scope=single_user` إلزامي حاليًا.

## فصل المستخدمين ومنع الخلط

من الآن، كل حوار في SF.AI يحمل هوية المستخدم حتى لو كان المستخدم الوحيد هو
سامي. الهدف هو منع خطأ معماري لاحقًا عند التوسع:

```text
export من مستخدم محدد
→ review/corpus بنفس owner_user_id
→ training/eval يعرف المصدر
→ runtime لاحقًا لا يرجع ذاكرة مستخدم إلى مستخدم آخر
```

المعرف المحلي الحالي:
...
أي سجل بلا هذه الحقول لا يعد training-ready.
...
- `owner_user_id=created_by_user_id=target_user_id=sami-local` في المسار الحالي.
- `user_scope=single_user`.
...
- حوارات تشغيلية أو هندسية أو خاصة بإدارة مشروع SF.AI.
- أوامر مثل: `التالي`, `اكمل`, `ارفع`, `phase`, `gates`, `corpus`,
  `tokenizer`, `pytest`, `commit`, `readiness`.
- أي agent workflow patterns أو تعليمات تشغيل للوكيل.
- أي persona خاصة بسامي أو طريقة إدارته للمشروع.
- بيانات بلا license.
...
أي سجل يحتوي هذا النمط يعد:

```text
training_forbidden_operational_internal_dialogue
```

ولا يجوز حفظه كـ review-only ولا تحويله لاحقًا. يجب حذفه من corpus
التدريبية واستبعاده من review exports وdialogue generation.
