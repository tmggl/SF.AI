...
- **الحالة الحالية:** المسار التدريبي أُعيد تثبيته رسميًا عند
  **Phase 27.79 — Objective/Curriculum/Decoding Repair Plan** ضمن
  `SF-native Objective/Curriculum/Decoding Acceleration Track`. لا تدريب
  جديد ولا tokenizer جديد ولا runtime release قبل مرور البوابات.
- **التركيز اللغوي الحالي:** العربية الفصحى + اللهجة السعودية فقط. لا توسيع للهجات أخرى في runtime قبل إتقان هذا المسار.
- **الهدف التالي العملي:** تشفير objective/curriculum/decoding gates ثم تنفيذ
  تدريب SF-10M محدود فقط إذا سمحت البوابات، دون قوالب تخفي الفشل.
...
| Phase 27.79 | Objective/Curriculum/Decoding Repair Plan | active re-anchor؛ `PHASE27_OBJECTIVE_CURRICULUM_DECODING_PLAN`, لا تدريب ولا runtime ولا SF-50M |
...
| Phase 27.85 | Explicit Family Conditioning Objective Design | مكتملة؛ صممت family conditioning، لا تدريب |
| Phase 27.86 | Family Conditioning Renderer Gate | مكتملة؛ renderer/masking نجح، يسمح بتدريب 27.87 فقط |
| Phase 27.87 | Bounded Family-conditioned SF-10M Repair Training | مكتملة؛ best=10/50، runtime محجوب، التشخيص مطلوب |
| Phase 27.88 | Family-conditioned Training Result Diagnosis | مكتملة؛ sequential curriculum collapse، لا تدريب |
| Phase 27.89 | Stratified Round-Robin Curriculum Sampler Gate | مكتملة؛ `family_round_robin` مرّ، تدريب 27.90 المقيّد مسموح فقط |
| Phase 27.90 | Bounded SF-10M Round-Robin Curriculum Repair Training | مكتملة؛ best=35/50، runtime محجوب، تشخيص 27.91 مطلوب |
| Phase 27.91 | Round-Robin Training Result Diagnosis | مكتملة؛ topic semantic collapse، لا تدريب |
| Phase 27.92 | Topic Objective Repair Design Gate | مكتملة؛ تصميم topic-objective جاهز، لا تدريب |
| Phase 27.93 | Topic Objective Gate Encoding and Dry-Run Validation | مكتملة؛ dry-run مر، ثم صار training-ready بعد 27.94 |
| Phase 27.94 | Topic Objective Data Pack Authoring | مكتملة؛ سدت فجوة الوفاء السعودية، تدريب 27.95 مسموح فقط |
| Phase 27.95 | Bounded Topic Objective Repair Training | مكتملة كتدريب مقيّد؛ فشلت البوابات، runtime محجوب والتشخيص مطلوب |
| Phase 27.96 | Topic Objective Repair Result Diagnosis | مكتملة كتشخيص؛ خلل ربط متغير الموضوع، لا تدريب |
| Phase 27.97 | Topic Variable Binding Objective Design | مكتملة كتصميم؛ objective copy/contrastive جاهز، لا تدريب |
| Phase 27.98 | Topic Binding Gate Encoding and Metadata Audit | مكتملة كبوابة؛ البيانات تحتاج إصلاح metadata، لا تدريب |
| Phase 27.99 | Topic Metadata and Copy-Anchor Data Repair | مكتملة؛ metadata/copy-anchor جاهزة، تدريب 27.100 المقيّد مسموح |
| Phase 27.100 | Bounded Topic Binding Repair Training | مكتملة كتدريب مقيّد؛ runtime محجوب والتشخيص مطلوب |
| Phase 27.101 | Topic Binding Repair Result Diagnosis | مكتملة كتشخيص؛ metric blind spot وtopic prototype attraction، لا تدريب |
| Phase 27.102 | Topic Prototype Contrastive Copy-Anchor Gate | مكتملة كبوابة؛ curriculum pack مسموح بلا تدريب |
| Phase 27.103 | Topic Prototype Contrastive Curriculum Pack | مكتملة كحزمة بيانات؛ تدريب 27.104 المقيّد مسموح، runtime محجوب |
| Phase 27.104 | Bounded Topic Prototype Contrastive Repair Training | مكتملة كتدريب؛ topic clean لكن all-family فشل، runtime محجوب |
...
- corpus بعد Phase 27.81: `8443` سجلًا، `issues=0`, `gold=3331`, `silver=5112`.
...
- فحص القرار:
  ...
  - corpus ready في ذلك الوقت: `8443`, issues=`0`.
...
- لم يبدأ تدريب جديد.
- لم يتغير runtime.
- صُممت صيغة family conditioning صريحة:
  - `النطاق: فصحى/سعودي`
...
- mapping:
  ...
  - `support → دعم`
  - `topic → موضوع`
- objective: أسطر conditioning وسطر المستخدم context فقط ومخفية عن loss؛ الهدف هو رد المساعد + EOS.
- القرار: `ALLOW_PHASE27_86_RENDERER_GATE_IMPLEMENTATION_NO_TRAINING`.
- التالي: `Phase 27.86 — Family Conditioning Renderer Gate`.
- محظور الآن: training, runtime release, SF-50M, tokenizer retrain.
...
- أضيف renderer مشترك `render_dialogue_text` ليستخدمه `ChatDataset` ومسار `train_tiny_lm` مع split-manifest.
- أصبح نص التدريب يرى:
  - `النطاق: فصحى/سعودي`
  - `عائلة الحوار: سوالف/متابعة/تنظيم/دعم/موضوع`
  - `المستخدم: ...`
  - `المساعد: ...`
- gate أثبت أن no-split وsplit-manifest يخرجان سطر family، وأن `loss_scope=assistant` يخفي conditioning/user lines ويشرف فقط رد المساعد + EOS.
- القرار: `ALLOW_PHASE27_87_BOUNDED_FAMILY_CONDITIONED_SF10M_REPAIR_TRAINING`.
- التالي: `Phase 27.87 — Bounded Family-conditioned SF-10M Repair Training`.
- محظور الآن: runtime release, SF-50M, tokenizer retrain, pretrained/open-weight usage.

### نتيجة Phase 27.87

- اكتمل تدريب SF-10M المقيّد بعد إصلاح renderer.
- لم يتغير runtime ولم تُفتح الواجهة.
- أضيف دعم `family_conditioning=True` في `NativeGeneratorConfig` للاختبار المخبري فقط، حتى يستخدم التوليد نفس سطر `عائلة الحوار` الذي رآه التدريب.
- أفضل نتيجة fresh shadow:
  - `sf-10m-step600`: `10/50`، أغلب النجاح في `open_social`.
  - `sf-10m-step1200`: `10/50`، أغلب النجاح في `planning`.
  - `sf-10m-step1800`: `7/50`، أغلب النجاح في `support`.
- القرار: `BLOCK_RUNTIME_DIAGNOSE_FAMILY_CONDITIONED_TRAINING_RESULT`.
- التالي: `Phase 27.88 — Family-conditioned Training Result Diagnosis`.
- محظور الآن: runtime release, SF-50M, tokenizer retrain, أي تدريب جديد قبل التشخيص.

### نتيجة Phase 27.88

- لم يبدأ تدريب جديد.
- لم يتغير runtime.
- شُخّص السبب الجذري لفشل 27.87: `sequential curriculum collapse`.
- أول 1800 عينة تدريب كانت موزعة بهذا الشكل:
  - `متابعة=451`
  - `سوالف=444`
  - `تنظيم=452`
  - `دعم=448`
  - `موضوع=5`
- هذا يعني أن التوازن العام في corpus لا يكفي؛ يجب توازن كل نافذة training/save.
- أوزان السبب: sequential ordering `38%`, checkpoint recency `22%`, topic underexposure `16%`, signal not interleaved `12%`, capacity `4%`.
- القرار: `DESIGN_STRATIFIED_ROUND_ROBIN_CURRICULUM_BEFORE_ANY_TRAINING`.
...
- لم يبدأ تدريب جديد.
- أضيف خيار `--split-order family_round_robin` إلى `train_tiny_lm`.
- الترتيب الجديد يستخدم `iter_split_samples_round_robin_by_family` لتقديم عائلات الحوار الخمس المعلّمة بالتناوب قبل العينات غير الموسومة.
- dry-run لأول 1800 عينة:
  - `open_social=360`
  - `followup=360`
  - `planning=360`
  - `support=360`
  - `topic=360`
- كل نافذة 600 تحتوي `120` من كل family، وdominant share = `0.20`.
...
- محظور الآن: runtime release, UI generator release, SF-50M, tokenizer retrain, pretrained/open-weight usage.

### نتيجة Phase 27.90

- دُرّب SF-10M تدريبًا محدودًا بعد gate 27.89.
- استخدم التدريب `--split-order family_round_robin`.
...
- محظور الآن: runtime release, UI generator release, SF-50M, tokenizer retrain, أي تدريب جديد قبل التشخيص.

### نتيجة Phase 27.91

- لم يبدأ تدريب جديد.
- شُخّص best checkpoint من 27.90: `sf-10m-step1800`.
- الفشل المتبقي:
  - topic: `9/15`
  - support: `3/15`
  - followup: `2/15`
...
- أوزان السبب: topic semantic collapse `48%`, topic underlearning `18%`, eval alias gap `12%`, surface artifacts `8%`, decoding repetition `6%`, capacity `4%`.
- القرار: `DESIGN_TOPIC_OBJECTIVE_REPAIR_GATE_BEFORE_ANY_TRAINING`.
- التالي: `Phase 27.92 — Topic Objective Repair Design Gate`.
- محظور الآن: training, runtime release, UI generator release, SF-50M, tokenizer retrain.

### نتيجة Phase 27.92

- لم يبدأ تدريب جديد.
- حُوّل تشخيص 27.91 إلى تصميم objective محدد لعائلة `topic`.
- objective: `topic_anchor_prompt_to_answer_objective_v1`.
- conditioning المطلوب:
  - `النطاق: <فصحى|سعودي>`
  - `عائلة الحوار: موضوع`
  - `الموضوع المطلوب: <topic_term>`
- مصطلحات البداية: `الوفاء`, `التعاون`, `الصبر`, `الاحترام`, `الهدوء`, `الصدق`, `الصداقة`, `الشجاعة`.
- canary gates قبل أي runtime: known topic `18/20`, fresh shadow `16/20`, all-family regression `45/50`, topic minimum `8/10`, malformed/repetition `0`.
...
- محظور الآن: training, runtime release, UI generator release, SF-50M, tokenizer retrain, pretrained/open-weight usage.

### نتيجة Phase 27.93

- لم يبدأ تدريب جديد.
- أضيف سطر renderer لعائلة topic فقط:
  - `الموضوع المطلوب: <topic_term>`
- السطر يظهر قبل `المستخدم:` ويُعامل كسياق masked تحت `loss_scope=assistant`.
- أضيف canary manifest:
  - `eval/prompts/phase27_93_topic_objective_canary.json`
- dry-run passed: `true`.
- training data ready: `false`.
- فجوة البيانات:
  - `الوفاء.total_shortfall=8`
  - `الوفاء.saudi_shortfall=10`
...
- لم يتغير runtime.
- لم يدرّب tokenizer جديد.
- أضيفت حزمة `sf-ai-topic-objective-wafa-saudi-pack-v1`:
  - `10` سجلات سعودية `gold`.
  - `dialogue_family=topic`.
  - `topic_term=الوفاء`.
  - source/license/quality/training_allowed موثقة داخل كل سجل.
- corpus الحالي بعد الحزمة: `8453` سجلًا، `issues=0`,
  `msa=4199`, `saudi=4254`, `gold=3341`, `silver=5112`.
- split الحالي: `train=7603`, `eval=850`.
- أعيد تشغيل Phase 27.93 gate بعد الحزمة:
  - `dry_run_passed=true`
  - `training_data_ready=true`
  - `shortfalls={}`
  - `الوفاء.total=22`, `الوفاء.msa=12`, `الوفاء.saudi=10`
- القرار: `ALLOW_PHASE27_95_BOUNDED_TOPIC_OBJECTIVE_REPAIR_TRAINING`.
- التالي: `Phase 27.95 — Bounded Topic Objective Repair Training`.
- محظور الآن: runtime release, UI generator release, SF-50M, tokenizer retrain,
  pretrained/open-weight usage.

### نتيجة Phase 27.95

- بدأ تدريب SF-10M مقيّد بعد قرار 27.94، ولم يتغير runtime.
- استخدم التدريب:
  - tokenizer: `artifacts/tokenizers/sf_bpe/v9_phase27_76`.
  ...
  - شرط topic renderer: `الموضوع المطلوب: <topic_term>`.
- checkpoints المقاسة:
  - `sf-10m-step600`: known topic `4/16`, fresh topic `2/10`, all-family `25/50`.
  - `sf-10m-step1200`: known topic `7/16`, fresh topic `2/10`, all-family `33/50`.
  - `sf-10m-step1800`: known topic `10/16`, fresh topic `4/10`, all-family `33/50`.
- البوابات المطلوبة لم تمر:
  - known topic المطلوب `16/16`.
  - fresh topic المطلوب `8/10`.
  - all-family المطلوب `45/50`.
- القرار: `PHASE27_95_BOUNDED_TOPIC_OBJECTIVE_REPAIR_DECISION`
  و`BLOCK_RUNTIME_DIAGNOSE_TOPIC_OBJECTIVE_REPAIR_RESULT`.
- محظور الآن: runtime release, UI generator release, SF-50M, tokenizer retrain,
  pretrained/open-weight usage، وأي تدريب جديد قبل التشخيص.
...
- لم يبدأ تدريب جديد.
- لم يتغير runtime.
- شُخّص فشل Phase 27.95 كخلل ربط متغير الموضوع داخل الرد.
- الإشارات:
  - known topic بقي `10/16`.
  - fresh topic بقي `4/10`.
  - all-family بقي `33/50`.
  - topic failures لم يحجبها الحارس: `guard_blocked_topic_failures=0`.
  - النموذج استبدل الموضوع المطلوب بموضوع خاطئ `11` مرة.
...
- أوزان السبب:
  ...
  - `model_capacity=3%`
- القرار: `PHASE27_96_TOPIC_OBJECTIVE_RESULT_DIAGNOSIS_DECISION`
  و`DESIGN_TOPIC_COPY_CONTRASTIVE_OBJECTIVE_BEFORE_ANY_TRAINING`.
- التالي: `Phase 27.97 — Topic Variable Binding Objective Design`.
- محظور الآن: training, runtime release, UI generator release, SF-50M,
...
- لم يتغير runtime.
- لم يتغير tokenizer.
- صُمم objective جديد لعلاج خلل ربط متغير الموضوع:
  `topic_copy_contrastive_binding_objective_v1`.
- عقد الهدف:
  - رد المساعد يجب أن ينسخ الموضوع المطلوب داخل أول 12 حرفًا عربيًا ظاهرًا.
  - الصيغ المقبولة تبدأ مثل: `معنى <topic_term>:` أو `<topic_term> يعني`
    أو `<topic_term> هو`.
  - لا يجوز أن يظهر موضوع محمي آخر قبل الموضوع المطلوب في إجابة topic قصيرة.
  - الرد جملة واحدة مدمجة، 8-18 كلمة عربية، مع EOS.
- بوابات canary المطلوبة للمرحلة اللاحقة:
  - known topic `16/16`.
  - fresh topic `8/10`.
  - contrastive wrong-topic max `0`.
  - copy-anchor `26/26`.
  - all-family regression `45/50`.
- القرار: `PHASE27_97_TOPIC_VARIABLE_BINDING_OBJECTIVE_DESIGN_DECISION`
  و`ALLOW_PHASE27_98_TOPIC_BINDING_GATE_ENCODING_NO_TRAINING`.
...
- لم يتغير runtime.
- لم يتغير tokenizer.
- رُمزت بوابة `topic_copy_contrastive_binding_objective_v1`.
- النتائج:
  - `encoded_gate_passed=true`
  - renderer/masking/canary جاهزة.
  - canary جديد: `26` حالة (`16` known + `10` fresh).
  - total topic records: `510`
  - explicit `topic_term`: `10`
  - missing `topic_term`: `500`
- القرار: `PHASE27_98_TOPIC_BINDING_GATE_ENCODING_DECISION`
  و`ALLOW_PHASE27_99_TOPIC_METADATA_COPY_ANCHOR_DATA_REPAIR_NO_TRAINING`.
...
- بعد إعادة بوابة Phase 27.98:
  ...
  - `copy_anchor_bad=0`
  - `unknown_topic=0`
- القرار: `PHASE27_99_TOPIC_METADATA_COPY_ANCHOR_REPAIR_DECISION`
  و`ALLOW_PHASE27_100_BOUNDED_TOPIC_BINDING_REPAIR_TRAINING`.
- التالي: `Phase 27.100 — Bounded Topic Binding Repair Training`.
- محظور الآن: runtime release, UI generator release, SF-50M,
  tokenizer retrain, pretrained/open-weight usage.

### نتيجة Phase 27.100

- نُفذ تدريب مقيّد على `SF-10M` فقط من checkpoint:
...
- النتائج:
  ...
  - topic family: `6/10`
  - all-family: `37/50`
- القرار: `PHASE27_100_BOUNDED_TOPIC_BINDING_REPAIR_DECISION`
  و`BLOCK_RUNTIME_DIAGNOSE_TOPIC_BINDING_REPAIR_RESULT`.
...
- لم يبدأ تدريب جديد.
- شُخّصت نتيجة Phase 27.100 مباشرة من التقرير والعينات.
- كشف التشخيص أن عداد `wrong_topic_count=0` في 27.100 كان blind spot:
  السبب `required_topic_missing` كان يسبق `wrong_topic_leak`، بينما فحص الردود
  مباشرة وجد observed wrong-topic `8`.
- بدائل الموضوع المرصودة:
  - `الصداقة=7`
  - `الامتنان=1`
- النتائج الأساسية:
  - known topic: `13/16`
...
- القرار: `PHASE27_101_TOPIC_BINDING_RESULT_DIAGNOSIS_DECISION`
  و`DESIGN_TOPIC_PROTOTYPE_CONTRASTIVE_COPY_ANCHOR_GATE_BEFORE_ANY_TRAINING`.
- التالي: `Phase 27.102 — Topic Prototype Contrastive Copy-Anchor Gate`.
- محظور الآن: runtime release, UI generator release, SF-50M,
  tokenizer retrain, pretrained/open-weight usage, وأي تدريب جديد قبل gate 27.102.

### نتيجة Phase 27.102

- لم يبدأ تدريب جديد.
- رُمزت بوابة observed wrong-topic/copy-anchor كأداة تنفيذية:
  `make phase27-topic-prototype-contrastive-gate`.
- البوابة أعادت قراءة 27.100 وأمسكت blind spot:
...
- أُنشئت ملفات:
  ...
  - `artifacts/reports/phase27_102_topic_prototype_contrastive_gate_spec.json`
- القرار: `PHASE27_102_TOPIC_PROTOTYPE_CONTRASTIVE_GATE_DECISION`
  و`ALLOW_PHASE27_103_TOPIC_PROTOTYPE_CONTRASTIVE_CURRICULUM_PACK_NO_TRAINING`.
...
- أضيفت ملفات:
  ...
  - `docs/PHASE27_103_TOPIC_PROTOTYPE_CONTRASTIVE_CURRICULUM_PACK_REPORT.md`
...
- التالي التاريخي كان `Phase 27.105 — Topic Prototype Repair Result Diagnosis`.
  بعد re-anchor الحالي، أصبحت `Phase 27.80` بوابة تنفيذية للتأكد من
  objective/curriculum/decoding، والتالي العملي بعد مرورها هو
  `Phase 27.81 — Execute bounded SF-10M family-conditioned repair training`.
- محظور الآن: runtime release, UI generator release, SF-50M,
  tokenizer retrain, pretrained/open-weight usage، وأي تدريب جديد قبل بوابات 27.80.

### Active Re-Anchor — Phase 27.79 Objective/Curriculum/Decoding Repair Plan

- أمر سامي الصريح يغيّر مسار التدريب الحالي إلى:
  `SF-native Objective/Curriculum/Decoding Acceleration Track`.
- التقرير الملزم: `PHASE27_OBJECTIVE_CURRICULUM_DECODING_PLAN`.
- المصدر الهندسي: Phase 27.104 نجحت في topic gates لكنها فشلت في
  all-family `30/50`; هذا يعني أن المشكلة ليست capacity بل objective,
  family mixing, curriculum order, decoding, وsemantic routing.
- الصيغة الرسمية للتدريب القادم:
  ```text
  النطاق: سعودي
  عائلة الحوار: تنظيم
  ...
  ```
- loss يُحسب على رد المساعد و`<eos>` فقط.
- context lines تدخل النموذج لكنها لا تدخل target loss.
- curriculum يجب أن يكون `stratified round-robin` بين:
  `open_social`, `followup`, `planning`, `support`, `topic`.
- decoding policy قبل الحكم على أي checkpoint:
  `stop_at_eos`, `no_repeat_ngram`, `repetition_penalty`,
  `known_fragment_blocklist`, `topic_substitution_guard`,
  `family_drift_guard`, و`template_masking_forbidden`.
- checkpoint selector لا يعتمد loss وحدها؛ يعتمد held-out dialogue quality,
  family stability, semantic correctness, clean-stop, open_social,
  followup, وruntime usability.
- AMP/MPS لا تُفعّل في التدريب إلا بعد smoke test واستقرار.
- TensorBoard/CSV logs محلية فقط.
- LoRA/QLoRA مؤجلة ومسموحة فقط على نماذج SF-native.
- DPO/ORPO/SimPO مؤجلة حتى توجد preference pairs محلية سيادية.
- محظور الآن: training عشوائي، tokenizer retrain، `SF-50M`,
  runtime release، template masking، pretrained/open-weight usage.
...
## الحالة العملية الحالية — Phase 27.119
...
- Phase 27.119 نفذ reference extraction dry-run counts فقط:
  `3010` candidate rows، quality bands input `916/703/1391`،
  `1093` reference candidates بعد الفلاتر، و`685` eval candidates.
  لا raw terms، لا reference records، لا corpus، لا tokenizer، لا training.
- لا تدريب جديد قبل Phase 27.120 local reference layer build gate ولا قبل بوابات
