...
```text
training_records: 280
target_records: 500
remaining_records: 220
dialect_counts: {"msa": 200, "saudi": 80}
...
batch_size: 25
estimated_batches: 9
quota_by_dialect: {"saudi": 120}
flexible_records_after_minimums: 100
planned_batches: 9
next_batch: saudi_003, dialect=saudi, target_records=25
```

`phase22-next-batch` يعرض المهمة الفورية للتأليف/المراجعة:

- batch الحالي: `saudi_003`.
...
- يذكر النواقص الحالية مثل `corpus_below_phase22_target`, `dialect_balance_below_minimum`, و`complete_next_batch:saudi_003`.
...
- `saudi_001` و`saudi_002` اكتملتا: 50 سجلًا سعوديًا owner-delegated agent-authored.
- `saudi_003` إلى `saudi_006`: تغطية سعودية، 25 سجلًا لكل batch.
...
```bash
make prepare-dialogue-batch ARGS="--input data/corpus/chat/review/<file>.jsonl --out data/corpus/chat/jsonl/dialogue_batch_v2_saudi_003.jsonl --quality silver --dialect saudi --training-allowed"
```
