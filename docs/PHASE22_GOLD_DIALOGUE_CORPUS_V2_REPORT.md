...
target_records: 500
remaining_records: 245
dialect_counts: {"msa": 200, "saudi": 55}
...
quota_by_dialect: {"saudi": 145}
flexible_records_after_minimums: 100
planned_batches: 10
next_batch: saudi_002, dialect=saudi, target_records=25
```

`phase22-next-batch` يعرض المهمة الفورية للتأليف/المراجعة:

- batch الحالي: `saudi_002`.
...
- يذكر النواقص الحالية مثل `corpus_below_phase22_target`, `dialect_balance_below_minimum`, و`complete_next_batch:saudi_002`.
...
- `saudi_001` اكتملت: 25 سجلًا سعوديًا owner-delegated agent-authored.
- `saudi_002` إلى `saudi_006`: تغطية سعودية، 25 سجلًا لكل batch.
...
```bash
make prepare-dialogue-batch ARGS="--input data/corpus/chat/review/<file>.jsonl --out data/corpus/chat/jsonl/dialogue_batch_v2_saudi_002.jsonl --quality silver --dialect saudi --training-allowed"
```
