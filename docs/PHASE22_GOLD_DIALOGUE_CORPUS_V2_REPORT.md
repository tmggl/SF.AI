...
```text
training_records: 425
target_records: 500
remaining_records: 75
dialect_counts: {"msa": 212, "saudi": 213}
...
estimated_batches: 3
quota_by_dialect: {}
flexible_records_after_minimums: 75
planned_batches: 3
next_batch: flex_002, dialect=msa_or_saudi, target_records=25
```

`phase22-next-batch` يعرض المهمة الفورية للتأليف/المراجعة:

- batch الحالي: `flex_002`.
...
- يذكر النواقص الحالية مثل `corpus_below_phase22_target` و`complete_next_batch:flex_002`. التوازن اللهجي الأساسي مكتمل حاليًا، لذلك لا تظهر `dialect_balance_below_minimum`.
...
- `flex_001` اكتملت: 25 سجلًا مرنًا بعد اكتمال الحد الأدنى.
- `flex_002` إلى `flex_004`: 75 سجلًا مرنًا متبقيًا.
...
```bash
make prepare-dialogue-batch ARGS="--input data/corpus/chat/review/<file>.jsonl --out data/corpus/chat/jsonl/dialogue_batch_v2_flex_002.jsonl --quality silver --dialect saudi --training-allowed"
```
