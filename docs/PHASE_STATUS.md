...
- **حالة المرحلة الحالية:** **بوابة جاهزية corpus v2 + review intake تعمل؛ corpus غير جاهز بعد (305/500، msa=200، saudi=105)**
...
- بدأ Phase 22 Gold Dialogue Corpus v2 كبوابة جاهزية:
  ...
  - الموجود الحالي: 305 سجل تدريب جاهز: 200 `msa` و105 `saudi`.
  - المتبقي: 195 سجل للوصول إلى 500.
  - خطة الجمع الحالية: 95 سعودي + 100 مرنة، أي 8 batches تقديريًا ومخططًا.
  - أضيفت بوابة اكتمال صارمة: `phase22-completion-gate`، وقرارها الحالي `PHASE22_INCOMPLETE_DO_NOT_ADVANCE`.
  - أضيفت قائمة batches مفصلة داخل `phase22-plan`: `saudi_004` إلى `saudi_007`، ثم `flex_001` إلى `flex_004`، مع target records وأسماء ملفات وأوامر تحقق مباشرة.
  - أضيفت مهمة batch فورية داخل `phase22-next-batch`: المهمة الحالية `saudi_004`، 25 سجلًا سعوديًا، مع checklist قبول وموضوعات تأليف لا تُعد بيانات تدريب.
  ...
  - أضيفت ثلاث دفعات سعودية معتمدة: `data/corpus/chat/jsonl/dialogue_batch_v2_saudi_001.jsonl` إلى `dialogue_batch_v2_saudi_003.jsonl` بإجمالي 75 سجلًا سعوديًا `silver`.
  ...
  - أضيفت لوحة مهمة الجمع الحالية داخل `/ui/chat`، تقرأ `/system/phase22-next-batch` وتعرض `saudi_004` وهدف 25 سجلًا وموضوعات تأليف عامة؛ الواجهة مختبر اختياري وليست شرطًا على سامي لحفظ أو تصدير أي شيء.
  ...
  - الناقص الحاسم الآن: العدد والتوازن؛ الفصحى وصلت إلى 200/200، والسعودي ما زال 105/200.
  - لا تدريب جديد بدأ.
