...
- بدأ Phase 19 كبوابة جاهزية قبل تدريب SF-50M:
  ...
  - السبب: corpus الحالي بعد Batch 003 صار `1550` سجل فقط والحد الأدنى العملي الحالي `5000`.
...
- بدأ وانتهى Phase 26 SF-50M v0.1 Readiness:
  ...
  - corpus كان `500` سجل عند قرار Phase 26، ثم صار `1550` بعد Batch 003؛ ما زال بعيدًا عن الحد العملي `5000`.
...
- بدأ وانتهى Phase 27 Dialogue Evaluation v2 + Corpus Expansion Plan:
  ...
  - خطة corpus الأصلية: `500 → 5000`. بعد Batch 003 صار المتبقي `3450`, عدد batches الكبيرة `7`, بالتوازن `msa=1725`, `saudi=1725`.
...
- بدأت توسعة corpus بعد Phase 27:
  ...
  - corpus الحالي صار `1550`: `msa=775`, `saudi=775`.
  - المتبقي إلى هدف `5000` صار `3450` سجلًا، أي `7` دفعات كبيرة بحجم 500 تقريبًا.
