...
- بدأ Phase 19 كبوابة جاهزية قبل تدريب SF-50M:
  ...
  - السبب: corpus الحالي بعد تنظيف الحوارات التشغيلية صار `643` سجل فقط والحد الأدنى العملي الحالي `5000`.
...
- بدأ وانتهى Phase 26 SF-50M v0.1 Readiness:
  ...
  - corpus كان `500` سجل عند قرار Phase 26، ثم صار `643` بعد تنظيف الحوارات التشغيلية؛ ما زال بعيدًا عن الحد العملي `5000`.
...
- بدأ وانتهى Phase 27 Dialogue Evaluation v2 + Corpus Expansion Plan:
  ...
  - خطة corpus الأصلية: `500 → 5000`. بعد تنظيف الحوارات التشغيلية صار المتبقي `4357`, عدد batches الكبيرة `9`, بالتوازن `msa=2201`, `saudi=2156`.
...
- بدأت توسعة corpus بعد Phase 27:
  ...
  - corpus الحالي صار `643`: `msa=299`, `saudi=344`.
  - المتبقي إلى هدف `5000` صار `4357` سجلًا، أي `9` دفعات كبيرة بحجم 500 تقريبًا.
