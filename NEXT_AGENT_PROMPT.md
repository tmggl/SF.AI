...
- المراحل من Phase 0 حتى Phase 27.79 موثقة تاريخيًا. الحالة الحالية:
  `Phase 27.81 — Balanced Family Pack Authoring` اكتملت بلا تدريب وأصدرت
  `PHASE27_81_BALANCED_FAMILY_PACK_DECISION`. أضيفت دفعة
  `sf-ai-balanced-family-pack-v1`: `2500` سجل gold متوازن (`500` لكل
  family و`250/250` فصحى/سعودي). corpus الحالي `8443`
  (`msa=4199`, `saudi=4244`, `gold=3331`, `silver=5112`). أعيد بناء split
  ومرّت Phase 27.80 repair gates. لا runtime switch ولا `SF-50M` ولا تدريب
  جديد حتى Phase 27.82 decision.
- أول خطوة تالية: Phase 27.82 Family-conditioned SF-10M repair training decision.
