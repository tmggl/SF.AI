...
- المراحل من Phase 0 حتى Phase 21 منتهية؛ Phase 22 تعمل الآن كبوابة جاهزية وخطة جمع وreview intake لـ Gold Dialogue Corpus v2. قرار Phase 22 الحالي: `NOT_READY_BUILD_GOLD_DIALOGUE_CORPUS_V2` لأن corpus الحالي 425/500 فقط (`msa=212`, `saudi=213`). خطة الجمع الحالية: 75 مرنة. استخدم `make phase22-next-batch` لمعرفة المهمة الفورية؛ الحالية `flex_002` بعدد 25 سجلًا.
...
- 435 اختبار يمر حاليًا (`.venv/bin/python -m pytest tests`) وآخر تشغيل: `435 passed in 5.04s`.
